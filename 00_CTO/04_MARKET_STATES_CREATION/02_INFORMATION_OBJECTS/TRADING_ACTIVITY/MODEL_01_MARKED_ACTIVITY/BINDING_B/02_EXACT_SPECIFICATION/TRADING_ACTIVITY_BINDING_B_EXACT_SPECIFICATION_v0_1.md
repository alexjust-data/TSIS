...
| `document_status` | `DRAFT_AMENDED_10_OF_12_RESOLVED_NOT_FROZEN` |
...
| `b02_decision_review` | `TRADING_ACTIVITY_BINDING_B_B02_DECISION_RESOLUTION_AND_SCIENTIFIC_REVIEW_v0_1.md` |
| `b02_decisions_resolved` | `10/12` |
| `b02_decisions_open` | `B02-D07, B02-D12` |
| `d07_label_contract_draft` | `WAKE_UP_LABEL_AND_NEGATIVE_DEFINITION_CONTRACT_v0_1.md` |
| `d07_label_output_contract_draft` | `WAKE_UP_LABEL_AND_DENOMINATOR_MATERIALIZATION_CONTRACT_v0_1.md` |
| `d12_selection_contract_draft` | `TRADING_ACTIVITY_VALIDATION_AND_FINAL_OOS_SELECTION_AND_SEAL_CONTRACT_v0_1.md` |
| `d12_manifest_schema` | `TRADING_ACTIVITY_LOCKBOX_MANIFEST_SCHEMA_v0_1.json` |
...
Classification: `RESOLVED_B02_DRAFT`; `B02-D02`.

```text
zero_session_count_B
= count_s(r_(s,m) = 0)

baseline_zero_fraction_B
= zero_session_count_B / B

ZERO_DOMINATED
= 2 * zero_session_count_B >= B
```

Therefore the exact integer thresholds are 10/20, 30/60 and 60/120 for
`B20`, `B60` and `B120`. Equality is deliberately fail-closed and no library
quantile convention may redefine this state.
...
Frozen draft threshold:

```text
theta_fast = 0.10 expected clusters
```

Diagnostic-only sensitivities are `0.05` and `0.20`. They cannot replace the
primary threshold after any validation result. `B02-D01` is resolved.

### 7.4 Silence-break surprise

Classification: `RESOLVED_B02_DRAFT`; `B02-D03`.
...
If the exact `B` prior sessions are unavailable, the value is `NULL` with
`BASELINE_INSUFFICIENT_HISTORY`. An inactive current interval is numeric zero
with `observation_state = OBSERVED_ZERO`; it is not missing or unavailable.
...
### 9.3 Short/long kernel ratios without a scientific epsilon

Classification: `RESOLVED_B02_DRAFT`; `B02-D04`.

```text
if the common event history is empty:
    kernel_log_ratio_2s_60s = 0
    observation_state = OBSERVED_ZERO

elif K_2s > 0 and K_60s > 0:
    kernel_log_ratio_2s_60s = log_K_2s - log_K_60s

else:
    terminal consistency failure
```

The kernels must be maintained in a stable scaled or log-domain state so that
underflow cannot turn positive common history into a false zero. Numerical
comparison tolerances belong to the implementation and replay contracts; they
must never appear as a floor in the scientific formula.
...
`BASELINE_INSUFFICIENT_POSITIVE_SUPPORT`. `U_B60` is explicitly conditional on
positive activity and may remain calculable when the unconditional context is
`ZERO_DOMINATED`. The following companions are mandatory:

```text
economic_unit_positive_support_count
economic_unit_positive_support_fraction
```
...
## 15. Physical family topology and cardinality

Classification: `RESOLVED_B02_DRAFT`; `B02-D06`.
...
This count is not copied as a scientific fact from A; it is the frozen draft B
storage topology. One and only one cardinality authority must generate expected
target identities, partitions, rows per family and total rows. The writer,
monitor, resume controller, aggregator, terminal certifier and independent
auditor must all consume that same authority.
...
Classification: `RESOLVED_B02_DRAFT` except for the false-activation label and
counting authority in `B02-D07`.

The exact governed input counts are:

```text
Binding A primary inputs = 97
B-full primary inputs    = 22
B-renewal-only inputs    = 10
B-kernel-only inputs     = 8
```

The 97-input A inventory and its physical schema fingerprints are frozen in
`TRADING_ACTIVITY_BINDING_A_STATISTICAL_INPUT_INVENTORY_FOR_B_COMPARISON_v0_1.md`.

### 18.1 Primary detector

```text
model = L2-regularized discrete-time logistic/hazard
loss  = mean negative log likelihood + lambda / 2 * ||beta||^2
...
All preprocessing is fit on development only. Continuous inputs use robust
median/IQR scaling. Numeric `NULL` values use the development median after
scaling and retain separate one-hot typed-reason indicators. The intercept is
unpenalized. Post-validation feature removal or promotion is prohibited.

At the fitted solution:

```text
effective_df
= trace((X' W X + lambda * P)^-1 X' W X) - 1
```

`P` has zero in the intercept position and one for penalized coefficients; the
subtracted one excludes the intercept. A fitted head is eligible only when
`effective_df <= 12`. The same grid, rule and search budget apply to A and all
B views.

### 18.2 Sensitivity detector

```text
model            = shallow gradient-boosted tree
max_depth        = {2, 3}
n_estimators     = {50, 100}
learning_rate    = {0.03, 0.10}
min_samples_leaf = {500, 2000}
total grid       = 16 combinations
```

No early stopping may inspect temporal validation. A and B receive the same
grid and trial count. This head is confirmatory and cannot rescue B if B fails
the primary detector criteria.
...
Frozen draft margins:

```text
recall loss no worse than 2 percentage points
median delay improvement >= max(1 second, 10 percent of A median delay)
coverage loss no worse than 1 percentage point
p95 delay loss <= max(2 seconds, 10 percent of A validation p95 delay)
```

The p95 constraint uses the upper bound of the paired 95 percent interval for
`B_p95 - A_p95`. Median improvement uses its lower bound; recall and coverage
use their adverse lower bounds.

Paired uncertainty is a moving-block bootstrap over governed XNYS
`session_date`, carrying all ticker-sessions for every selected date. It uses
blocks of five consecutive governed sessions, 5,000 replicates, a two-sided
95 percent percentile interval and seed `20260815`. Development and temporal
validation are bootstrapped separately and never mixed. Symbol-seconds are not
treated as independent observations. `B02-D08`, `D09` and `D10` are resolved.
...
Classification: `RESOLVED_FOR_B02_DRAFT`; measured B-06/B-08 evidence remains
...
The machine is the governed 8-core/16-thread Ryzen 7 5800X with 32 GB RAM. The
static limits and escalation rule are frozen in
`TRADING_ACTIVITY_BINDING_B_B02_COMPUTE_BUDGET_ANNEX_v0_1.md`: initial maximum
two workers, process-tree RSS at most 24 GiB, at least 6 GiB reserved for the
OS/monitor, projected full wall time at most 72 hours, projected final output at
most 64 GiB, launch free disk at least `max(3 * projected output, 100 GiB)`,
heartbeat age at most 60 seconds and atomic checkpoint interval at most 15
minutes. B-06/B-08 must measure and certify the projection before B-12. Native
optimization remains optional and requires oracle equivalence. `B02-D11` is
resolved for B-02 only.

## 22. Decision status before B-02 freeze

| ID | Decision | Current proposed value | State |
|---|---|---|---|
| `B02-D01` | fast operational-duration threshold | primary `0.10`; diagnostic `0.05/0.20` | `RESOLVED_B02_DRAFT` |
| `B02-D02` | zero-dominated rule | `2 * zero_session_count_B >= B` | `RESOLVED_B02_DRAFT` |
| `B02-D03` | silence-break formula | Laplace-smoothed one-second activity probability plus typed insufficient-history branch | `RESOLVED_B02_DRAFT` |
| `B02-D04` | kernel ratio | branch/log-domain formula; no scientific epsilon | `RESOLVED_B02_DRAFT` |
| `B02-D05` | economic-unit support | 20/60 positive observations plus conditional-support companions | `RESOLVED_B02_DRAFT` |
| `B02-D06` | family topology/cardinality | three families; 7,200 partitions; `5 * D_s` logical rows | `RESOLVED_B02_DRAFT` |
| `B02-D07` | false-activation authority and budget | label/counting architecture drafted; numerical oracle, timing, labels and budget require human freeze/materialization | `BLOCKING_B02_FREEZE` |
| `B02-D08` | exact detector grids and capacity cap | L2 grid; `effective_df <= 12`; 16-combination confirmatory GBT | `RESOLVED_B02_DRAFT` |
| `B02-D09` | p95 delay non-inferiority margin | `max(2 seconds, 10 percent of A p95)` | `RESOLVED_B02_DRAFT` |
| `B02-D10` | paired uncertainty design | 5-session moving blocks; 5,000 reps; 95 percent; seed `20260815` | `RESOLVED_B02_DRAFT` |
| `B02-D11` | compute budget | governed static annex; later measured B-06/B-08 gate required | `RESOLVED_FOR_B02_DRAFT` |
| `B02-D12` | exact final-lockbox manifest identity | selection/seal contract and schema drafted; target size/salt/custodian plus exact artifacts pending | `BLOCKING_B02_FREEZE` |
...
B-02 exact-specification draft    = AMENDED_NOT_FROZEN
B-02 decisions                    = 10 RESOLVED / 2 OPEN
B-02 open blockers                = B02-D07, B02-D12
...
The next action is explicit human freeze of the open label/counting and
selection/custody decisions, followed by Data Foundation label/denominator
contracts and independent custodian generation of sealed artifacts. Code
creation remains prohibited until D07/D12 close and a separate explicit B-02
freeze opens B-03.
