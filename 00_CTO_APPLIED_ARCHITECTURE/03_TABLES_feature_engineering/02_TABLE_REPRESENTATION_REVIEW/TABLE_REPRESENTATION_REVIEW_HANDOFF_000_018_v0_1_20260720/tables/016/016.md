...
## Regla de perfiles fisicos

`market_state_table_v0_1` no debe interpretarse como una mega-tabla universal.

La identidad canonica de Market State debe poder sostener varios perfiles fisicos compatibles:
...
Todos los perfiles deben compartir, como minimo:
...
```text
El core no es la union de todos los atributos disponibles.
El core contiene la semantica minima gobernada del estado.
Las extensiones contienen informacion pesada, especializada o de consumo condicionado.
```

Esta ficha no autoriza todavia esos perfiles como datasets fisicos oficiales.
Solo fija la frontera conceptual que debe respetar el futuro mapping/schema/builder.
