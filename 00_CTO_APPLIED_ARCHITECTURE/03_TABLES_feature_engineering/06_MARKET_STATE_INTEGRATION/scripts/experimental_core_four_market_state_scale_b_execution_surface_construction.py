...
SCRIPT_VERSION = "experimental_core_four_market_state_scale_b_execution_surface_construction_v0_1"
RUN_ID_PREFIX = "experimental_core_four_market_state_scale_b_execution_surface_construction_v0_1"
DEFAULT_SCOPE = (
    Path(__file__).resolve().parents[1]
    / "configs"
    / "experimental_core_four_market_state_scale_b_execution_surface_construction_scope_v0_1.json"
)
SURFACE_NAME = "014_scale_b_execution_surface_candidate_v0_1.parquet"
...
def validate_scope(scope: dict[str, Any]) -> None:
    if scope.get("scope_id") != "experimental_core_four_market_state_scale_b_execution_surface_construction_scope_v0_1":
        raise SurfaceConstructionError("Unexpected scope_id")
    if scope.get("mode") != "experimental_core_four_market_state_scale_b_execution_surface_construction":
        raise SurfaceConstructionError("Unexpected mode")
    authority = scope.get("authority", {})
    required_true = [
        "scale_b_execution_surface_construction_allowed",
...
def load_sample(scope: dict[str, Any], scope_dir: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    expected_fp = sample_info["scale_b_sample_fingerprint"]
    ...
    return rows, {
        ...
        "scale_b_sample_fingerprint": observed_fp,
        "tickers": tickers,
        "sessions": sessions,
    }
...
def build_surface_rows(
    *,
    ...
    rows["selection_reason"] = "scale_b_frozen_sample_governed_calendar_session_surface"
...
def write_run_readout(path: Path, summary: dict[str, Any], run_dir: Path) -> None:
    text = f"""# Experimental Core Four Market State Scale B Execution Surface Construction Readout v0.1
...
```text
experimental_core_four_market_state_scale_b_execution_surface_construction = {summary['surface_construction_status']}
scale_b_builder_resolution_execution = NOT_EXECUTED_REQUIRES_SEPARATE_GATE
...
def main() -> int:
    ...
    write_json(run_dir / "scale_b_source_snapshot_manifest.json", source_snapshot_payload)
    ...
    surface_manifest = {
        ...
        "scale_b_sample_fingerprint": sample_manifest["scale_b_sample_fingerprint"],
        "calendar_selection_fingerprint": calendar_manifest["calendar_selection_fingerprint"],
        "source_snapshot_fingerprint": source_snapshot_fingerprint,
        "scale_b_execution_surface_fingerprint": surface_semantic_fingerprint,
        "parquet_sha256": surface_sha256,
        "parquet_bytes": int(surface_path.stat().st_size),
    }
    write_json(run_dir / "scale_b_execution_surface_manifest.json", surface_manifest)
    ...
    write_csv_rows(
        run_dir / "scale_b_surface_source_coverage_report.csv",
        [
            "source_alias",
            "rows_read",
    ...
    write_csv_rows(
        run_dir / "scale_b_surface_calendar_binding_report.csv",
        [
            "session_date",
            "matched_rows",
            "calendar_id",
    ...
    write_csv_rows(
        run_dir / "scale_b_surface_session_boundary_report.csv",
        [
            "ticker",
            "session_date",
            "calendar_version",
    ...
    write_csv_rows(
        run_dir / "scale_b_surface_duplicate_report.csv",
        [
            "ticker",
            "session_date",
            "bar_end_utc",
    ...
    write_csv_rows(
        run_dir / "scale_b_surface_cutoff_capability_report.csv",
        [
            "sample_context_id",
            "ticker",
            "instrument_id",
    ...
    write_json(run_dir / "scale_b_surface_determinism_report.json", determinism_payload)
    authority_report = {
        "run_id": run_id,
    ...
    write_json(run_dir / "scale_b_surface_authority_report.json", authority_report)
    ...
    summary = {
        ...
        else "remediate_scale_b_execution_surface_construction",
    }
    write_json(run_dir / "scale_b_execution_surface_summary.json", summary)
    write_run_readout(
        run_dir / "experimental_core_four_market_state_scale_b_execution_surface_construction_readout_v0_1.md",
        summary,
        run_dir,
    )
    ...
    missing_required_outputs = sorted(expected_outputs - set(observed_outputs) - {"scale_b_execution_surface_final_manifest.json"})
    final_outputs = sorted(set(observed_outputs) | {"scale_b_execution_surface_final_manifest.json"})
    ...
    if missing_required_outputs or output_file_count > int(scope["limits"]["maximum_output_files"]):
        ...
        if status == "PASS_WITH_RESTRICTIONS":
            ...
            summary["next_allowed_gate"] = "remediate_scale_b_execution_surface_construction"
            write_json(run_dir / "scale_b_execution_surface_summary.json", summary)
    ...
    write_json(run_dir / "scale_b_execution_surface_final_manifest.json", final_manifest)
    heartbeat("completed", status=final_manifest["status"], final_manifest=str(run_dir / "scale_b_execution_surface_final_manifest.json"))
