...
SCRIPT_VERSION = "experimental_core_four_market_state_scale_b_market_state_integration_execution_v0_1"
DEFAULT_SCOPE = (
    Path(__file__).resolve().parents[1]
    / "configs"
    / "experimental_core_four_market_state_scale_b_market_state_integration_execution_scope_v0_1.json"
)


def validate_authority(scope: dict[str, Any]) -> None:
    if scope.get("mode") != "experimental_core_four_market_state_scale_b_market_state_integration_execution":
...
def validate_builder_manifest(
    ...
    exact_keys = {
        "scale_b_sample_fingerprint": bindings["scale_b_sample_fingerprint"],
        "scale_b_execution_surface_fingerprint": bindings["scale_b_execution_surface_fingerprint"],
...
def validate_resolution_record_bindings(
    ...
    for record in records:
        ...
        if temporal_lineage.get("calendar_version") != bindings["calendar_version"]:
            counts["calendar_binding_mismatches"] += 1
        if lineage.get("scale_b_execution_surface_fingerprint") != bindings["scale_b_execution_surface_fingerprint"]:
            counts["surface_fingerprint_mismatches"] += 1
        if lineage.get("scale_b_sample_fingerprint") != bindings["scale_b_sample_fingerprint"]:
...
def add_scale_b_lineage(
    ...
    for candidate in candidates:
        ...
        candidate["lineage"] = {
            ...
            "scale_b_sample_run_id": bindings["scale_b_sample_run_id"],
            "scale_b_sample_fingerprint": bindings["scale_b_sample_fingerprint"],
            "scale_b_execution_surface_run_id": bindings["scale_b_execution_surface_run_id"],
            "scale_b_execution_surface_fingerprint": bindings["scale_b_execution_surface_fingerprint"],
            "calendar_binding_run_id": bindings["calendar_binding_run_id"],
            "calendar_version": bindings["calendar_version"],
            "calendar_source_snapshot_fingerprint": bindings["calendar_source_snapshot_fingerprint"],
...
def write_findings(path: Path, summary: dict[str, Any]) -> None:
    text = f"""# Scale B Market State Integration Execution Findings v0.1
...
```text
experimental_core_four_market_state_scale_b_market_state_integration_execution = {summary["experimental_core_four_market_state_scale_b_market_state_integration_execution"]}
...
The execution integrated only accepted Scale B core-four resolution records from
...
```text
experimental_core_four_market_state_scale_b_candidate_materialization_authorization_v0_1 = CONDITIONAL_NEXT_AUTHORIZATION_GATE
...
def write_readout(path: Path, summary: dict[str, Any]) -> None:
    text = f"""# Experimental Core Four Market State Scale B Market State Integration Execution Readout v0.1

Status: `{summary["experimental_core_four_market_state_scale_b_market_state_integration_execution"]}`
Date: `2026-07-23`
Run: `{summary["run_id"]}`

```text
source_builder_run_id = {summary["source_builder_run_id"]}
scale_b_sample_fingerprint = {summary["scale_b_sample_fingerprint"]}
scale_b_execution_surface_fingerprint = {summary["scale_b_execution_surface_fingerprint"]}
...
def main(argv: list[str] | None = None) -> int:
    ...
    pre_manifest = {
        ...
        "mode": scope["mode"],
        "execution_class": "experimental_non_materializing_scale_b_artifact_integration",
        "input_scope_path": str(scope_path),
        "output_root": str(run_root),
        "run_dir": str(run_dir),
        "expected_scope": "integrate accepted Scale B core-four resolution records only; no source market data reads",
    ...
    if len(records) != int(expected["input_resolution_records"]):
        raise ProbeError("Input resolution record count does not match expected Scale B cardinality")
    if grouped_count != int(expected["contexts_seen"]):
        raise ProbeError("Input context count does not match expected Scale B cardinality")
    ...
    add_scale_b_lineage(candidates, records, scope)
    ...
    if (
        ...
        overall_status = "passed_scale_b_market_state_integration_execution_with_restrictions"
    else:
        gate_status = "FAILED"
        overall_status = "failed_scale_b_market_state_integration_execution"
    ...
    summary_path = run_dir / "scale_b_integration_summary.json"
    findings_path = run_dir / "scale_b_integration_findings.md"
    ...
    summary = {
        ...
        "experimental_core_four_market_state_scale_b_market_state_integration_execution": gate_status,
        "profile_id": scope["profile_id"],
        "source_builder_run_id": scope["source_builder_run_id"],
        "scale_b_sample_fingerprint": scope["accepted_bindings"]["scale_b_sample_fingerprint"],
        "scale_b_execution_surface_fingerprint": scope["accepted_bindings"][
        ...
        "next_allowed_gate": "experimental_core_four_market_state_scale_b_candidate_materialization_authorization_v0_1",
        ...
        "artifacts": {
            ...
            "scale_b_integration_summary": str(summary_path),
            "scale_b_integration_findings": str(findings_path),
