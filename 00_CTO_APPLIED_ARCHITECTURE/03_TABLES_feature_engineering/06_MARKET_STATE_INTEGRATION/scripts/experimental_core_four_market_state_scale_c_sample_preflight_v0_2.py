...
DEFAULT_SCOPE = (
    ...
    / "configs"
    / "experimental_core_four_market_state_scale_c_scope_v0_2.json"
)
RUN_ID_PREFIX = "experimental_core_four_market_state_scale_c_sample_preflight_v0_2"
EXPECTED_OBJECTS_PER_CONTEXT = 4
EXPECTED_SOURCE_ALIASES = {"004_master_daily_table", "013_ohlcv_1m_quote_guarded"}
...
def validate_authority(scope: dict[str, Any]) -> None:
    if scope.get("scope_id") != "experimental_core_four_market_state_scale_c_scope_v0_2":
    ...
    if scope.get("scope_status") != "authorized_with_restrictions_sample_preflight_only":
        raise PreflightError(f"Scale C scope is not authorized for preflight: {scope.get('scope_status')}")
    authority = scope.get("authority", {})
    required_true = [
        "scale_c_sample_preflight_authorized",
        ...
        "historical_candidate_pool_discovery_authorized",
        "bounded_daily_historical_pool_discovery_read_allowed",
    ]
    required_false = [
        "scale_c_execution_authorized",
        "scale_c_execution_surface_construction_authorized",
    ...
    if aliases != EXPECTED_SOURCE_ALIASES:
        raise PreflightError(f"Unexpected source aliases allowed for Scale C preflight: {sorted(aliases)}")
    forbidden = {str(item).replace("\\", "/") for item in scope.get("source_aliases_forbidden", [])}
    if "00_CTO/99_REFERENCE_LIBRARY" not in forbidden:
        raise PreflightError("Reference library exclusion is missing from Scale C scope")
    if "014_master_intraday_bar_table_candidate_as_builder_input" not in forbidden:
        raise PreflightError("014 builder-input exclusion is missing from Scale C scope")
...
def resolve_source_roots(scope: dict[str, Any], scope_dir: Path) -> tuple[Path, dict[str, Path], dict[str, Any]]:
    ...
    for alias in sorted(EXPECTED_SOURCE_ALIASES):
...
def load_calendar(scope: dict[str, Any], scope_dir: Path) -> tuple[pd.DataFrame, list[dict[str, Any]], dict[str, Any]]:
    ...
    for plan in scope["historical_session_plan"]:
        ...
        plan_rows.append(
            {
                ...
                "finding": "governed calendar boundary matches authorized Scale C historical session plan"
                if boundary_match and version_match
                else "governed calendar row does not match authorized Scale C historical session plan",
            }
        )
    report = {
...
def load_eligible_pool(scope: dict[str, Any], scope_dir: Path) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    report = {
        ...
        "pool_use": "lineage_only_not_scale_c_candidate_universe",
        ...
        "candidate_instrument_pool_cap": int(pool_cfg["candidate_instrument_pool_cap"]),
    }
    return normalized[: int(pool_cfg["candidate_instrument_pool_cap"])], report
...
def governed_required_daily_dates(calendar: pd.DataFrame, session_dates: list[str], prior_sessions: int) -> list[str]:
    calendar_dates = sorted(
        {
            parsed.isoformat()
            for value in calendar["session_date"].tolist()
            for parsed in [parse_session_date(value)]
            if parsed is not None
        }
    )
    required: set[str] = set(session_dates)
    for session_date in session_dates:
        previous = [item for item in calendar_dates if item < session_date]
        required.update(previous[-prior_sessions:])
    return sorted(required)


def read_daily_historical_candidate_source(
    root: Path,
    ...
    missing = [column for column in columns if column not in available]
    if missing:
        raise PreflightError(f"004 missing required historical pool columns: {missing}")
    date_values = [date.fromisoformat(item) for item in required_dates]
    years = sorted({int(item[:4]) for item in required_dates})
    filt = (
        ds.field("year").isin([int(year) for year in years])
        & (ds.field("price_view") == "split_normalized")
        & ds.field("session_date").isin(date_values)
    )
    table = dataset.to_table(columns=columns, filter=filt)
    df = table.to_pandas().reset_index(drop=True)
    df["ticker"] = df["ticker"].astype(str).str.strip().str.upper()
    ...
    report = {
        ...
        "rows_read": int(len(df)),
        "columns_read": columns,
        "required_daily_dates": required_dates,
        "years_filter": years,
        "ticker_filter": "all_tickers_bounded_by_governed_required_daily_dates",
        "selected_price_view": "split_normalized",
        "unique_tickers": int(df["ticker"].nunique(dropna=True)),
        "unique_sessions": int(df["session_date_iso"].nunique(dropna=True)),
    }
...
def build_historical_candidate_pool(
    *,
    ...
    _, daily_by_ticker = daily_maps(daily)
    session_set = set(session_dates)
    report_rows: list[dict[str, Any]] = []
    candidates: list[dict[str, Any]] = []
    for ticker in sorted(daily_by_ticker):
        rows = daily_by_ticker[ticker]
        instrument_id, identity_resolved, identity_ambiguous, identity_count = resolve_instrument_id(rows)
        target_rows = [row for row in rows if row["session_date"] in session_set]
        target_dates_present = {row["session_date"] for row in target_rows}
        daily_linked_count = sum(
            1
            for row in target_rows
            if row.get("instrument_id")
            and row.get("open_present")
            and row.get("prior_close_present")
            and row.get("volume_present")
        )
        prior20_count = sum(1 for session_date in session_dates if prior_volume_count(rows, session_date) >= 20)
        required_field_count = daily_linked_count
        daily_history_candidate = bool(
            identity_resolved
            and not identity_ambiguous
            and len(target_dates_present) == len(session_dates)
            and daily_linked_count == len(session_dates)
            and prior20_count == len(session_dates)
            and required_field_count == len(session_dates)
        )
        doc = {
            "ticker": ticker,
            ...
            "daily_target_session_count": len(target_dates_present),
            ...
            "daily_history_candidate": daily_history_candidate,
            "historical_candidate_rank": "",
            "selected_for_intraday_feasibility_preflight": False,
            "finding": "daily history candidate across all governed Scale C sessions"
            if daily_history_candidate
            else "not a daily history candidate across all governed Scale C sessions",
        }
        report_rows.append(doc)
        if daily_history_candidate:
            candidates.append(
                {
                    "ticker": ticker,
                    "instrument_id": instrument_id,
                    "source_pool_rank": 0,
                    "daily_history_candidate": True,
                }
            )
    ranked = sorted(candidates, key=lambda row: (str(row["ticker"]), str(row["instrument_id"])))
    for rank, row in enumerate(ranked, start=1):
        row["source_pool_rank"] = rank
    selected = ranked[:candidate_cap]
    selected_tickers = {row["ticker"] for row in selected}
    selected_rank = {row["ticker"]: row["source_pool_rank"] for row in selected}
    for row in report_rows:
        if row["ticker"] in selected_tickers:
            row["historical_candidate_rank"] = selected_rank[row["ticker"]]
            row["selected_for_intraday_feasibility_preflight"] = True
...
def discover_013_files(root: Path, tickers: list[str], session_dates: list[str]) -> tuple[list[Path], dict[str, Any]]:
    year_months = sorted({(int(session_date[:4]), int(session_date[5:7])) for session_date in session_dates})
    files: list[Path] = []
    ...
    report = {
        ...
        "physical_candidate_root": str(root),
        "partition_lookup_mode": "bounded_by_pool_ticker_and_authorized_historical_session_month",
...
def read_013_files(files: list[Path], allowed_dates: set[str]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    for path in files:
        ...
        frames.append(df)
        file_docs.append({"path": str(path), "rows": int(len(df)), "sha256": sha256_file(path), "read_filter": "bounded_candidate_month_partition"})
    if frames:
    ...
    report = {
        "source_alias": "013_ohlcv_1m_quote_guarded",
        "read_mode": "bounded_candidate_month_partitions_then_authorized_session_filter",
        "rows_read": rows_read,
            ...
            coverage_rows.append(
                {
                    ...
                    "source_coverage_status": "PASS" if eligible_session else "BLOCKED_SOURCE_PERIOD_COVERAGE",
                    "finding": "daily, prior-20 and intraday feasibility coverage present for governed historical session"
                    if eligible_session
                    else "missing daily, prior-20, identity or intraday feasibility coverage for governed historical session",
                }
            )
        status = "ELIGIBLE_FOR_SCALE_C_SAMPLE" if eligible_session_count == len(session_dates) else "NOT_ELIGIBLE_FOR_SCALE_C_SAMPLE"
        doc = {
            "ticker": ticker,
            "instrument_id": pool_instrument_id,
            ...
            "selection_rank": "",
            "selected_for_scale_c_sample": False,
            "finding": "eligible across all governed Scale C historical sessions"
            if status == "ELIGIBLE_FOR_SCALE_C_SAMPLE"
            else "not eligible across all governed Scale C historical sessions",
        }
...
def build_contexts(
    *,
    ...
    target_instruments = int(shape["selected_instruments_target"])
    ...
    selected = selected_instruments[:target_instruments]
    if len(selected) < target_instruments:
        return []
    ...
    def add_context(
        *,
        ...
        rows.append(
            {
                "sample_context_id": f"scale_c_context_{context_index:04d}",
                ...
                "calendar_row_fingerprint": cal.get("calendar_row_fingerprint", ""),
                ...
                "source_coverage_status": "PASS" if coverage_pass.get((instrument["ticker"], session_date)) else "BLOCKED_SOURCE_PERIOD_COVERAGE",
    ...
    for instrument in selected:
        for session_date in session_dates:
            ...
            add_context(
                ...
                selection_reason="one governed historical-session first-observable context for every selected instrument-session pair",
            )
    if len([row for row in rows if row["decision_case_family"] == "instrument_session_core_context"]) != int(
        allocation["instrument_session_core_contexts"]
    ):
        return []
    ...
    for instrument in selected:
        summary = intraday_summaries.get((instrument["ticker"], last_session))
    ...
    stress_target = int(allocation["historical_period_boundary_stress_contexts"])
    blocked_stress_target = int(scope["target_sample_shape"]["expected_blocked_contexts"]) - int(
        allocation["pre_bar_expected_blocked_contexts"]
    )
    stress_added = 0
    for idx, instrument in enumerate(selected):
        for offset in (2, 5):
            if stress_added >= stress_target:
                break
            session_date = session_dates[(idx + offset) % len(session_dates)]
            cal = calendar_by_date[session_date]
            summary = intraday_summaries.get((instrument["ticker"], session_date))
            if not summary or summary.get("first_bar_utc") is None:
                return []
            stress_added += 1
            if stress_added <= blocked_stress_target:
                decision_timestamp = pd.Timestamp(summary["first_bar_utc"]) - pd.Timedelta(seconds=1)
                decision_case = "historical_period_pre_open_boundary_blocked"
                expected_context_outcome = "REJECTED_REQUIRED_OBJECT_BLOCKED"
                expected_materialized_as_row = False
                reason = "blocked historical-period stress context before first observable governed-session bar"
            else:
                decision_timestamp = parse_utc_timestamp(f"{session_date}T{cal['observed_close_utc']}Z")
                decision_case = "governed_historical_session_close_boundary"
                expected_context_outcome = "INTEGRABLE_COMPLETE_OR_WITH_RESTRICTIONS"
                expected_materialized_as_row = True
                reason = "integrable historical-period stress context at governed session close"
            add_context(
                instrument=instrument,
                session_date=session_date,
                decision_case_family="historical_period_boundary_stress_context",
                decision_case=decision_case,
                decision_timestamp=decision_timestamp,
                expected_context_outcome=expected_context_outcome,
                expected_materialized_as_row=expected_materialized_as_row,
                selection_stratum="historical_period_boundary_stress_context",
                selection_reason=reason,
            )
        if stress_added >= stress_target:
            break
    if len([row for row in rows if row["decision_case_family"] == "historical_period_boundary_stress_context"]) != stress_target:
        return []
...
def build_duplicate_context_report(sample_rows: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], int, int]:
    ...
    if not sample_rows:
        rows.append(
            {
                "check_name": "scale_c_sample_duplicate_contexts",
                "sample_context_id": "",
...
def build_historical_coverage_rows(
    calendar_rows: list[dict[str, Any]],
    coverage_rows: list[dict[str, Any]],
    ...
    for cal in calendar_rows:
        rows = by_session.get(cal["session_date"], [])
        output.append(
            {
                "session_date": cal["session_date"],
                "calendar_stratum": cal["stratum"],
                "selected_instruments": len(selected_tickers),
                "daily_linked_instruments": sum(1 for row in rows if row["daily_linked"]),
                "prior_20_complete_instruments": sum(1 for row in rows if row["prior_20_daily_volume_coverage_available"]),
                "intraday_present_instruments": sum(1 for row in rows if row["intraday_session_present"]),
                "required_field_complete_instruments": sum(1 for row in rows if row["required_core_four_source_fields_present"]),
                "source_coverage_failures": sum(1 for row in rows if row["source_coverage_status"] != "PASS"),
                "calendar_boundary_match": cal.get("calendar_boundary_match"),
                "calendar_version_match": cal.get("calendar_version_match"),
                "status": "PASS"
                if rows and all(row["source_coverage_status"] == "PASS" for row in rows) and cal.get("compatible") is True
                else "BLOCKED_SOURCE_PERIOD_COVERAGE",
            }
        )
    return output
...
def decide_status(summary: dict[str, Any]) -> str:
    ...
    if summary["calendar_session_boundary_mismatches"] > 0 or summary["selected_sessions"] != summary["required_sessions"]:
        return "BLOCKED_CALENDAR_PERIOD_PLAN"
    if summary["eligible_instruments"] < summary["required_instruments"]:
        return "BLOCKED_INSTRUMENT_HISTORY_COVERAGE"
    if summary["sample_manifest_rows"] != summary["requested_contexts"]:
        return "BLOCKED_SAMPLE_CARDINALITY"
    if summary["source_coverage_failures"] > 0 or summary["historical_source_coverage_failures"] > 0:
        return "BLOCKED_SOURCE_PERIOD_COVERAGE"
    if summary["identity_history_failures"] > 0:
        return "BLOCKED_INSTRUMENT_HISTORY_COVERAGE"
    if summary["formula_history_coverage_failures"] > 0:
        return "BLOCKED_FORMULA_HISTORY_COVERAGE"
...
def write_readout(path: Path, *, run_id: str, summary: dict[str, Any], run_dir: Path) -> None:
    ...
    lines = [
        ...
        "",
        "```text",
        f"experimental_core_four_market_state_scale_c_sample_preflight = {status}",
        f"experimental_core_four_market_state_scale_c_execution = {summary['scale_c_execution_status']}",
        ...
        f"selected_period_years = {summary['selected_period_years']}",
        ...
        f"minimum_instruments = {summary['minimum_instruments']}",
        ...
        f"observed_expected_blocked_contexts = {summary['observed_expected_blocked_contexts']}",
        f"observed_expected_integrable_contexts = {summary['observed_expected_integrable_contexts']}",
        ...
        f"historical_source_coverage_failures = {summary['historical_source_coverage_failures']}",
        f"formula_history_coverage_failures = {summary['formula_history_coverage_failures']}",
        f"identity_history_failures = {summary['identity_history_failures']}",
    ...
    if status == "PASS_WITH_RESTRICTIONS":
        lines.extend(
            [
                ...
                "",
                "```text",
                f"scale_c_sample_fingerprint = {summary['scale_c_sample_fingerprint']}",
...
def main(argv: list[str] | None = None) -> int:
    ...
    pre_manifest = {
        ...
        "mode": scope["mode"],
        "execution_class": "experimental_scale_c_sample_preflight",
        "input_scope_path": str(scope_path),
        "run_dir": str(run_dir),
        "expected_scope": "freeze 120 governed-calendar Scale C contexts after bounded historical candidate-pool discovery only if calendar, identity and source history coverage pass",
    ...
    lineage_pool_instruments, pool_report = load_eligible_pool(scope, scope_dir)
    registry_path, roots, registry = resolve_source_roots(scope, scope_dir)
    required_daily_dates = governed_required_daily_dates(
        calendar,
        [row["session_date"] for row in calendar_rows],
        int(scope["historical_candidate_pool_discovery"]["prior_sessions_required_per_target_session"]),
    )
    daily, daily_report = read_daily_historical_candidate_source(roots["004_master_daily_table"], required_daily_dates)
    historical_candidate_pool, historical_candidate_pool_rows = build_historical_candidate_pool(
        daily=daily,
        session_dates=[row["session_date"] for row in calendar_rows],
        candidate_cap=int(scope["historical_candidate_pool_discovery"]["candidate_tickers_for_intraday_feasibility_cap"]),
    )
    tickers = [item["ticker"] for item in historical_candidate_pool]
    ...
    intraday_summaries = build_intraday_summaries(intraday, calendar_rows)
    instrument_rows, coverage_rows, ranked_candidates = evaluate_instruments(
        pool_instruments=historical_candidate_pool,
        session_dates=[row["session_date"] for row in calendar_rows],
        daily=daily,
        intraday_summaries=intraday_summaries,
    )
    shape = scope["target_sample_shape"]
    required_instruments = int(shape["selected_instruments_target"])
    minimum_instruments = int(shape["selected_instruments_minimum"])
    ...
    for row in instrument_rows:
        if row["ticker"] in selected_tickers:
            row["selected_for_scale_c_sample"] = True
            row["instrument_selection_status"] = "SELECTED_FOR_SCALE_C_SAMPLE"
    ...
    if len(sample_rows) != int(shape["target_requested_contexts"]):
        sample_rows = []
    ...
    family_map = {
        ...
        "historical_period_boundary_stress_contexts": "historical_period_boundary_stress_context",
    }
    stratification_failures = 0
    if sample_rows:
        for family, expected in allocation.items():
            stratification_failures += int(sample_family_counts.get(family_map[family], 0) != int(expected))
    else:
    ...
    historical_rows = build_historical_coverage_rows(calendar_rows, coverage_rows, selected_tickers)
    historical_source_coverage_failures = sum(1 for row in historical_rows if row["status"] != "PASS")
    identity_history_failures = sum(1 for row in instrument_rows if row["selected_for_scale_c_sample"] and row["identity_matches_pool"] is not True)
    formula_history_coverage_failures = sum(
        1
        for row in coverage_rows
        if row["ticker"] in selected_tickers and row["prior_20_daily_volume_coverage_available"] is not True
    )
    calendar_boundary_mismatches = sum(1 for row in calendar_rows if row.get("compatible") is not True)
    fixed_utc_authority_count = 0
    observed_expected_blocked_contexts = sum(1 for row in sample_rows if row.get("expected_materialized_as_row") is False)
    observed_expected_integrable_contexts = sum(1 for row in sample_rows if row.get("expected_materialized_as_row") is True)
    selected_period_years = len({row["session_date"][:4] for row in calendar_rows})
    ...
    instrument_selection_fingerprint = sha256_payload(
        {
            "selected_instruments": [
                ...
                for row in instrument_rows
                if row["selected_for_scale_c_sample"]
            ],
    ...
    scale_c_sample_fingerprint = sha256_payload(sample_rows) if sample_rows else ""
    ...
    summary: dict[str, Any] = {
        ...
        "accepted_scale_b_physical_validation_run_id": scope["prerequisite_evidence"]["scale_b_candidate_physical_validation"]["run_id"],
        ...
        "lineage_seed_pool_instruments": int(pool_report["eligible_instruments_available"]),
        "historical_daily_candidate_pool_size": sum(1 for row in historical_candidate_pool_rows if row["daily_history_candidate"]),
        "historical_intraday_candidate_pool_cap": int(scope["historical_candidate_pool_discovery"]["candidate_tickers_for_intraday_feasibility_cap"]),
        "historical_intraday_candidate_pool_size": len(historical_candidate_pool),
        "seed_pool_instruments": len(historical_candidate_pool),
        "seed_pool_fingerprint": pool_report["eligible_instrument_pool_fingerprint"],
        "requested_contexts": int(shape["target_requested_contexts"]),
        "maximum_requested_contexts_respected": bool(len(sample_rows) <= int(shape["maximum_requested_contexts"])),
        ...
        "minimum_instruments_met": len(ranked_candidates) >= minimum_instruments,
        "eligible_instruments": len(ranked_candidates),
        "selected_instruments": len(selected_instruments) if sample_rows else 0,
        "required_sessions": int(shape["selected_sessions"]),
        "selected_sessions": len(session_dates),
        "selected_period_years": selected_period_years,
        "expected_resolution_records": int(shape["target_resolution_records_if_later_executed"]),
        "expected_blocked_contexts": int(shape["expected_blocked_contexts"]),
        "expected_integrable_contexts": int(shape["target_integrable_contexts"]),
        "observed_expected_blocked_contexts": int(observed_expected_blocked_contexts),
        "observed_expected_integrable_contexts": int(observed_expected_integrable_contexts),
        ...
        "historical_source_coverage_failures": int(historical_source_coverage_failures),
        "formula_history_coverage_failures": int(formula_history_coverage_failures),
        "identity_history_failures": int(identity_history_failures),
        ...
        "scale_c_sample_fingerprint": scale_c_sample_fingerprint,
        "builders_executed": False,
        "resolution_records_emitted": 0,
        "market_state_integration_executed": False,
        "market_state_materialization_executed": False,
    ...
    blocking_statuses = set(scope["blocked_statuses_allowed"])
    summary["hard_preflight_failures"] = 1 if summary["preflight_status"] in blocking_statuses else 0
    summary["scale_c_execution_status"] = (
        "NOT_EXECUTED_READY_FOR_SEPARATE_GATE"
        if summary["preflight_status"] == "PASS_WITH_RESTRICTIONS"
        else "BLOCKED_NOT_STARTED"
    )

    write_json(run_dir / "scale_c_calendar_period_plan.json", {"run_id": run_id, "sessions": calendar_rows})
    write_csv_rows(
        run_dir / "scale_c_calendar_coverage_report.csv",
        [
            "session_date",
            "stratum",
            "expected_open_utc",
    ...
    write_csv_rows(
        run_dir / "scale_c_historical_candidate_pool_report.csv",
        [
            "ticker",
            "instrument_id",
            "identity_resolved",
            "identity_ambiguous",
            "identity_count",
            "daily_target_session_count",
            "daily_linked_session_count",
            "prior_20_daily_volume_complete_session_count",
            "required_field_complete_session_count",
            "daily_history_candidate",
            "historical_candidate_rank",
            "selected_for_intraday_feasibility_preflight",
            "finding",
        ],
        historical_candidate_pool_rows,
    )
    write_csv_rows(
        run_dir / "scale_c_instrument_selection_report.csv",
        [
            "ticker",
            "instrument_id",
            "source_pool_rank",
    ...
    write_csv_rows(
        run_dir / "scale_c_source_coverage_preflight_report.csv",
        [
            "ticker",
            "instrument_id",
            "session_date",
    ...
    write_csv_rows(
        run_dir / "scale_c_historical_coverage_report.csv",
        [
            "session_date",
            "calendar_stratum",
            "selected_instruments",
            "daily_linked_instruments",
            "prior_20_complete_instruments",
            "intraday_present_instruments",
            "required_field_complete_instruments",
    ...
    write_csv_rows(
        run_dir / "scale_c_context_plan_report.csv",
        [
            "sample_context_id",
            "instrument_id",
            "ticker",
    ...
    write_csv_rows(
        run_dir / "scale_c_duplicate_context_report.csv",
    ...
    write_json(run_dir / "scale_c_sample_summary.json", summary)
    authority_report = {
        "run_id": run_id,
        ...
        "source_reports": {
            ...
            "scale_a_seed_pool_lineage": pool_report,
    ...
    write_json(run_dir / "scale_c_authority_report.json", authority_report)
    final_manifest = {
        "run_id": run_id,
    ...
    readout_path = integration_root / "experimental_core_four_market_state_scale_c_sample_preflight_readout_v0_2.md"
