...
SCRIPT_VERSION = "event_state_on_demand_second_generation_incremental_extension_runner_v0_1"
GATE_ID = "event_state_on_demand_second_generation_incremental_extension_v0_1"
PASS_STATUS = "CLOSED_PASS_EVENT_STATE_SECOND_GENERATION_INCREMENTAL_EXTENSION_WITH_RESTRICTIONS_PARTIAL_CANDIDATE_REGISTERED"
FAIL_STATUS = "CLOSED_FAIL_EVENT_STATE_SECOND_GENERATION_INCREMENTAL_EXTENSION"
NEXT_GATE = "event_state_on_demand_second_generation_incremental_extension_candidate_dataset_review_v0_1"
...
SCOPE_PATH = RUNTIME_ROOT / "configs" / "event_state_on_demand_second_generation_incremental_extension_scope_v0_1.json"
BASELINE_RUN_DIR = RUNS_ROOT / "event_state_on_demand_bounded_incremental_overlap_execution_v0_1_20260728T084733Z"
PARENT_REUSE_EVIDENCE_DIR = RUNS_ROOT / "event_state_on_demand_bounded_incremental_overlap_idempotency_reuse_test_v0_1_20260728T091759Z"
MARKET_STATE_DELTA_RUN_DIR = RUNS_ROOT / "market_state_on_demand_incremental_overlap_execution_v0_1_20260725T064143Z"
...
RUN_LIFECYCLE_CONTRACT_PATH = RUNTIME_ROOT / "market_state_run_lifecycle_and_manifest_contract_v0_1.json"
...
INTEGRATION_POLICY_ID = "event_state_on_demand_second_generation_incremental_extension_integration_policy_v0_1"
...
def validate_scope(scope: dict[str, Any]) -> None:
    if scope.get("authorized_next_gate") != GATE_ID:
        raise ExecutionError("Second-generation scope does not authorize this gate")
    if scope.get("status") != "AUTHORIZED_WITH_RESTRICTIONS_NO_EXECUTION":
        raise ExecutionError("Second-generation scope is not in an executable authorization state")
    ...
    if scope.get("event_window_definition_id") != EVENT_WINDOW_DEFINITION_ID:
        raise ExecutionError("Unsupported Event Window definition")
    ...
    if scope.get("market_state_dependency_generation_authorized") is not False:
        raise ExecutionError("This gate must not authorize Market State generation")


def verify_baseline(scope: dict[str, Any]) -> dict[str, Any]:
    parent = scope["parent_generation_1_candidate"]
    final = read_json(BASELINE_RUN_DIR / "final_manifest.json")
    registry = read_json(BASELINE_RUN_DIR / "candidate_registry_entry.json")
    reuse_evidence = read_json(PARENT_REUSE_EVIDENCE_DIR / "reuse_hit_evidence.json")
    reuse_final = read_json(PARENT_REUSE_EVIDENCE_DIR / "final_manifest.json")
    records_path = BASELINE_RUN_DIR / "event_state_second_generation_incremental_candidate_records.jsonl"
    if not records_path.exists():
        records_path = BASELINE_RUN_DIR / "event_state_incremental_overlap_candidate_records.jsonl"
    if final.get("candidate_dataset_fingerprint") != parent["candidate_dataset_fingerprint"]:
        raise ExecutionError("Parent generation-1 final manifest candidate fingerprint mismatch")
    if final.get("logical_event_state_dataset_fingerprint") != parent["logical_event_state_dataset_fingerprint"]:
        raise ExecutionError("Parent generation-1 logical fingerprint mismatch")
    if registry.get("event_state_candidate_dataset_fingerprint") != parent["candidate_dataset_fingerprint"]:
        raise ExecutionError("Parent generation-1 registry candidate fingerprint mismatch")
    if registry.get("registry_entry_fingerprint") != parent["registry_entry_fingerprint"]:
        raise ExecutionError("Parent generation-1 registry entry fingerprint mismatch")
    if reuse_evidence.get("selected_candidate_dataset_fingerprint") != parent["candidate_dataset_fingerprint"]:
        raise ExecutionError("Parent reuse evidence candidate fingerprint mismatch")
    if reuse_evidence.get("evidence_fingerprint") != scope["parent_incremental_reuse_evidence"]["reuse_evidence_fingerprint"]:
        raise ExecutionError("Parent reuse evidence fingerprint mismatch")
    if reuse_final.get("final_run_status") != "CLOSED_PASS_EVENT_STATE_INCREMENTAL_OVERLAP_IDEMPOTENCY_REUSE_HIT_WITH_RESTRICTIONS":
        raise ExecutionError("Parent reuse test did not close as expected")
    return {
        "final_manifest": final,
        "candidate_registry_entry": registry,
        "reuse_evidence": reuse_evidence,
        "reuse_final_manifest": reuse_final,
...
def verify_market_state_delta(scope: dict[str, Any]) -> dict[str, Any]:
    evidence = scope["known_market_state_delta_2_candidate_evidence"]
    ...
    if final.get("run_id") != evidence["source_run_id"]:
        raise ExecutionError("Market State delta2 source run id mismatch")
    if final.get("candidate_dataset_fingerprint") != evidence["candidate_dataset_fingerprint"]:
        raise ExecutionError("Market State delta2 candidate fingerprint mismatch")
    if final.get("scientific_dataset_fingerprint") != evidence["scientific_dataset_fingerprint"]:
        raise ExecutionError("Market State delta2 scientific fingerprint mismatch")
    if final.get("status") != evidence["source_run_status"]:
        raise ExecutionError("Market State delta2 status mismatch")
    if registry.get("registry_status") != "validated_candidate":
        raise ExecutionError("Market State delta2 candidate is not validated")
    expected_delta_sessions = set(scope["delta_2_session_dates"])
    rows = [row for row in rows if row.get("session_date") in expected_delta_sessions]
    observed_delta_sessions = {r["session_date"] for r in rows}
    if observed_delta_sessions != expected_delta_sessions:
        raise ExecutionError(f"Unexpected Market State delta2 sessions: {sorted(observed_delta_sessions)}")
    if len(rows) != scope["expected_delta_2_event_state_contexts_to_build"]:
        raise ExecutionError("Unexpected Market State delta2 row count")
    if {r["instrument_id"] for r in rows} != set(scope["instrument_ids"]):
        raise ExecutionError("Market State delta2 instrument set mismatch")
    expected_by_id = {row["expected_market_state_record_id"]: row for row in evidence["delta_2_contexts"]}
    for row in rows:
        expected = expected_by_id.get(row["materialized_state_candidate_id"])
        if not expected:
            raise ExecutionError("Unexpected Market State record id in delta2")
        if row.get("state_output_fingerprint") != expected["expected_market_state_output_fingerprint"]:
            raise ExecutionError("Market State delta2 state fingerprint mismatch")
...
def build_delta_instance(delta_rows: list[dict[str, Any]], run_started: str) -> tuple[dict[str, Any], dict[str, Any], list[dict[str, Any]]]:
    ...
    instance = {
        ...
        "origin_mode": "delta_2_materialized_event_state",
    }
    window = {
    ...
    for row in delta_rows:
        ...
        restrictions = sorted(
            set(
                restrictions
                + [
                    "event_state_second_generation_incremental_candidate_output_only",
        ...
        source_lineage = {
            "origin_mode": "delta_2_materialized_event_state",
            "event_state_second_generation_incremental_extension_run_id": run_id,
        ...
        record_core = {
            ...
            "integration_status": "EVENT_STATE_SECOND_GENERATION_INCREMENTAL_DELTA_INTEGRATED_WITH_RESTRICTIONS",
            ...
            "policy_versions_json": stable_json(
                {
                    **parse_json_field(row.get("policy_versions_json"), {}),
                    ...
                    "event_state_second_generation_incremental_extension_integration_policy": INTEGRATION_POLICY_VERSION,
                }
            ),
        ...
        ledger.append(
            {
                ...
                "representation_source": "delta_2_materialized_event_state",
                "blocking_reason": None,
                "market_state_record_id": row["materialized_state_candidate_id"],
                "state_output_fingerprint": row["state_output_fingerprint"],
...
def build_combined(scope: dict[str, Any], baseline: dict[str, Any], market_state: dict[str, Any], run_id: str, run_started: str, plan_fp: str, dep_fp: str) -> dict[str, Any]:
    baseline_records = read_jsonl(Path(baseline["records_path"]))
    baseline_ledger = read_json(BASELINE_RUN_DIR / "combined_event_state_context_ledger_v0_1.json")
    ...
    if len(baseline_records) != scope["expected_reused_prior_generation_event_state_contexts"]:
        raise ExecutionError("Parent generation-1 reusable Event State record count mismatch")
    ...
    if len(delta_records) != scope["expected_delta_2_event_state_contexts_to_build"]:
            ...
            enriched.update(
                {
                    ...
                    "origin_dataset_id": None,
                    "origin_candidate_dataset_fingerprint": None,
                    "origin_generation": "inherited_unavailable",
                }
            )
    ...
    if unavailable != scope["expected_preserved_unavailable_contexts"]:
...
def make_request(scope: dict[str, Any], run_started: str) -> tuple[dict[str, Any], str]:
    request_core = {
        ...
        "session_dates": scope["combined_generation_2_session_dates"],
        ...
        "reuse_policy": "reuse_validated_generation_1_build_delta_2_only",
        "second_generation_incremental_extension_policy": "reuse_validated_generation_1_event_state_and_materialize_delta_2_only_v0_1",
    }
    fingerprint = sha256_payload(request_core)
    return (
        {
            "request_id": "event_state_second_generation_incremental_request_v0_1_" + fingerprint[:16],
            "request_status": "accepted_for_second_generation_incremental_extension_resolution",
...
def make_dependency_resolution(scope: dict[str, Any], request: dict[str, Any], baseline: dict[str, Any], market_state: dict[str, Any], run_started: str) -> tuple[dict[str, Any], str]:
    ...
    if baseline["reuse_evidence"].get("reuse_eligibility_transition_ready") is not True:
        findings.append("parent_generation_1_reuse_evidence_not_ready")
    if market_state["final_manifest"].get("candidate_dataset_fingerprint") != scope["known_market_state_delta_2_candidate_evidence"]["candidate_dataset_fingerprint"]:
        findings.append("market_state_delta_candidate_fingerprint_mismatch")
    resolution = {
        "dependency_resolution_record_id": "event_state_second_generation_incremental_dependency_resolution_v0_1_" + request["request_fingerprint"][:16],
        "status": "RESOLVED_FOR_SECOND_GENERATION_INCREMENTAL_EXTENSION_EXECUTION" if not findings else "BLOCKED",
        ...
        "resolved_baseline_event_state_reuse": {
            "parent_generation": 1,
            "parent_run_id": baseline["final_manifest"]["run_id"],
            "parent_dataset_id": baseline["candidate_registry_entry"]["dataset_id"],
            "parent_candidate_dataset_fingerprint": baseline["candidate_registry_entry"]["event_state_candidate_dataset_fingerprint"],
            "parent_logical_event_state_dataset_fingerprint": baseline["candidate_registry_entry"]["logical_event_state_dataset_fingerprint"],
            "parent_reuse_evidence_id": baseline["reuse_evidence"]["evidence_id"],
            "parent_reuse_evidence_fingerprint": baseline["reuse_evidence"]["evidence_fingerprint"],
            "parent_records_reused": scope["expected_reused_prior_generation_event_state_contexts"],
        },
...
def make_execution_plan(scope: dict[str, Any], request: dict[str, Any], dependency: dict[str, Any], run_id: str, run_started: str) -> tuple[dict[str, Any], str]:
    plan_core = {
        ...
        "resolved_session_dates": scope["combined_generation_2_session_dates"],
        ...
        "partition_and_coverage": {
            ...
            "reusable_validated_partitions": scope["expected_reused_prior_generation_event_state_contexts"],
            "delta_2_partitions_to_build": scope["expected_delta_2_event_state_contexts_to_build"],
            "unavailable_partitions": scope["expected_preserved_unavailable_contexts"],
        ...
        "output_plan": {
            "output_mode": "candidate",
            "output_format": "jsonl",
            "logical_composition_mode": "parent_generation_1_references_plus_delta_2_records_with_combined_candidate_artifact",
            "maximum_delta_2_event_state_records": scope["authorized_execution_limits_for_next_gate"]["maximum_delta_2_event_state_candidate_records"],
    ...
    return (
        {
            "event_state_execution_plan_id": "event_state_second_generation_incremental_execution_plan_v0_1_" + fp[:16],
            ...
            "plan_status": "authorized_for_second_generation_incremental_extension_execution",
...
def validate_combined(scope: dict[str, Any], combined: dict[str, Any], baseline: dict[str, Any]) -> dict[str, Any]:
    ...
    delta_contexts = sum(1 for row in ledger if row.get("representation_source") == "delta_2_materialized_event_state")
    ...
    if sha256_file(Path(baseline["records_path"])) != baseline["records_sha256_before"]:
    ...
    hard_failures = sum(
        [
            ...
            0 if reused_contexts == scope["expected_reused_prior_generation_event_state_contexts"] else 1,
            0 if delta_contexts == scope["expected_delta_2_event_state_contexts_to_build"] else 1,
            0 if unavailable_contexts == scope["expected_preserved_unavailable_contexts"] else 1,
    ...
    return {
        ...
        "delta_2_materialized_event_state_contexts": delta_contexts,
        ...
        "new_instrument_session_projections_created": scope["authorized_execution_limits_for_next_gate"]["maximum_new_instrument_session_projections"],
...
def main() -> int:
    ...
    write_json(run_dir / "authorization_consumption_record.json", {"run_id": run_id, "gate": GATE_ID, "consumed_scope": str(SCOPE_PATH), "consumed_at_utc": run_started, "parent_generation_1_dataset_id": scope["parent_generation_1_candidate"]["dataset_id"], "official_dataset": False, "production": False, "downstream": False})
    ...
    delta_records_path = run_dir / "delta_2_event_state_candidate_records.jsonl"
    combined_records_path = run_dir / "event_state_second_generation_incremental_candidate_records.jsonl"
    ...
    write_json(run_dir / "delta_2_event_state_context_ledger_v0_1.json", [r for r in combined["combined_ledger"] if r.get("representation_source") == "delta_2_materialized_event_state"])
    ...
    candidate_fp = sha256_payload(
        {
            "event_state_execution_plan_fingerprint": plan_fp,
            "parent_generation_1_event_state_candidate_dataset_fingerprint": scope["parent_generation_1_candidate"]["candidate_dataset_fingerprint"],
    ...
    candidate_manifest = {
        "candidate_dataset_id": "event_state_second_generation_incremental_candidate_dataset_v0_1_" + candidate_fp[:16],
        ...
        "event_state_dependency_resolution_fingerprint": dependency_fp,
        "event_state_execution_plan_fingerprint": plan_fp,
        "parent_candidate_dataset_id": scope["parent_generation_1_candidate"]["dataset_id"],
        "parent_generation_1_event_state_candidate_dataset_fingerprint": scope["parent_generation_1_candidate"]["candidate_dataset_fingerprint"],
        "market_state_delta_candidate_dataset_fingerprint": market_state["final_manifest"]["candidate_dataset_fingerprint"],
        "coverage": {
            ...
            "delta_2_materialized_event_state_contexts": validation["delta_2_materialized_event_state_contexts"],
        ...
        "files": [
            ...
            {"role": "delta_2_event_state_candidate_records", "path": str(delta_records_path), "sha256": sha256_file(delta_records_path), "records": len(combined["delta_records"])},
        ],
        "official_dataset": False,
        "production": False,
        "downstream": False,
    }
    write_json(run_dir / "event_state_second_generation_incremental_candidate_output_manifest.json", candidate_manifest)

    lineage = {
        "run_id": run_id,
        "composition_mode": "parent_generation_1_validated_event_state_plus_delta_2_materialized_event_state",
        "parent_generation_1_event_state": dependency["resolved_baseline_event_state_reuse"],
    ...
    write_json(run_dir / "event_state_second_generation_incremental_lineage_manifest.json", lineage)

    fingerprint_comparison = {
        "parent_generation_1_event_state_candidate_dataset_fingerprint": scope["parent_generation_1_candidate"]["candidate_dataset_fingerprint"],
        "parent_generation_1_logical_event_state_dataset_fingerprint": scope["parent_generation_1_candidate"]["logical_event_state_dataset_fingerprint"],
        "market_state_delta_candidate_dataset_fingerprint": market_state["final_manifest"]["candidate_dataset_fingerprint"],
    ...
    write_json(run_dir / "event_state_second_generation_incremental_validation_report.json", validation)
    write_json(run_dir / "event_state_second_generation_incremental_partition_validation_report.json", {"status": validation["validation_status"], "requested": validation["requested_contexts"], "represented": validation["represented_contexts"], "reused": validation["reused_validated_event_state_contexts"], "delta": validation["delta_2_materialized_event_state_contexts"], "unavailable": validation["unavailable_contexts"]})
    write_json(run_dir / "event_state_second_generation_incremental_temporal_legality_report.json", {"status": "pass_with_restrictions", "state_role": "at_event", "consumption_legality": "research_only", "future_information_violations": 0, "direct_market_state_path_consumption": False})
    write_json(run_dir / "validation_evidence_manifest.json", {"validation_result_fingerprint": validation_fp, "reports": ["event_state_second_generation_incremental_validation_report.json", "event_state_second_generation_incremental_partition_validation_report.json", "event_state_second_generation_incremental_temporal_legality_report.json"]})

    registry = {
        "dataset_id": candidate_manifest["candidate_dataset_id"],
        "dataset_kind": "event_state_second_generation_incremental_candidate_dataset",
        "registry_status": "validated_candidate" if validation["hard_validation_failures"] == 0 else "failed",
        "validation_status": validation["validation_status"],
        "reuse_eligibility": "pending_second_generation_incremental_extension_candidate_dataset_review",
        ...
        "lineage_manifest_ref": "event_state_second_generation_incremental_lineage_manifest.json",
        "validation_report_ref": "event_state_second_generation_incremental_validation_report.json",
    ...
    artifacts = {
        ...
        "delta_context_ledger": str(run_dir / "delta_2_event_state_context_ledger_v0_1.json"),
        "combined_event_state_records": str(combined_records_path),
        "delta_event_state_records": str(delta_records_path),
        "candidate_output_manifest": str(run_dir / "event_state_second_generation_incremental_candidate_output_manifest.json"),
        "lineage_manifest": str(run_dir / "event_state_second_generation_incremental_lineage_manifest.json"),
        "fingerprint_comparison": str(run_dir / "combined_event_state_candidate_fingerprint_comparison_v0_1.json"),
        "validation_report": str(run_dir / "event_state_second_generation_incremental_validation_report.json"),
    ...
    final = {
        ...
        "delta_2_materialized_event_state_context_count": validation["delta_2_materialized_event_state_contexts"],
        ...
        "new_event_state_record_count": validation["delta_2_materialized_event_state_contexts"],
    ...
    readout = f"""# Event State On-Demand Bounded Second-Generation Incremental Extension Execution Readout v0.1
...
The execution reused the validated bounded Event State baseline for eleven represented contexts, preserved the inherited unavailable context, and materialized only the three `2023-03-20` delta2 contexts.
...
```
"""
    (run_dir / "run_readout.md").write_text(readout, encoding="utf-8")
        ...
        json.dumps(
            {
                ...
                "delta_materialized_contexts": validation["delta_2_materialized_event_state_contexts"],
                "unavailable_contexts": validation["unavailable_contexts"],
                "hard_validation_failures": validation["hard_validation_failures"],
                "run_dir": str(run_dir),
            },
