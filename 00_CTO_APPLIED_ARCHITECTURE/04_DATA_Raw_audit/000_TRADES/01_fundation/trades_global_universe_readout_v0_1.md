...
![Distribucion final](images/evidence_assets/global_universe/00_acceptance_distribution.png)
...
![Mezcla anual](images/evidence_assets/global_universe/01_yearly_acceptance_mix.png)
...
![Scale bucket mix](images/evidence_assets/global_universe/02_scale_bucket_mix_by_label.png)
...
- La imagen enseÃ±a que la escala no es un ruido pequeÃ±o repartido homogÃ©neamente. EstÃ¡ **estructurada por label**.
...
- Cuantitativamente, el bloque esta muy repartido:
  ...
  - `~1x = 5.81%`
  - `~15x = 5.63%`
- Eso significa que ni siquiera sumando `~10x` y `~20x` dominamos el bucket; el conflicto de escala es mas amplio y heterogeneo que un simple â€œfactor diezâ€.
...
![Firmas duras](images/evidence_assets/global_universe/03_signature_mix_by_label.png)
...
- Esta es una imagen de mezcla causal, no de tamaÃ±o bruto. Lo que importa no es solo que firma aparece, sino **en quÃ© label pesa relativamente mÃ¡s**.
...
- La presencia de `rows_lt_10` y `off_session_trades_present` importa porque introduce una segunda dimensiÃ³n: no todo conflicto nace en el precio o en la escala; parte del daÃ±o vive en **la forma del file**.
- La imagen tambiÃ©n enseÃ±a quÃ© firmas son marginales en ciertos labels. Eso es Ãºtil porque evita sobredimensionar una causa donde en realidad pesa poco.
...
![Outside daily](images/evidence_assets/global_universe/04_outside_daily_severity_by_label.png)
...
- La figura no solo dice quÃ© labels tienen mÃ¡s conflicto contra `daily`; dice **cÃ³mo se reparte internamente ese conflicto**.
- En `review`, el patrÃ³n es relativamente benigno si se compara con otros buckets:
...
- En `bad_data`, la mezcla es mas interesante:
  ...
  - `(20,100)` en `32.08%`
  - `0%` en `4.78%`
- Esa combinacion prueba que `bad_data` no es solo el bucket de â€œtodo fuera todo el ratoâ€; hay una franja relevante donde el conflicto es duro pero no total.
- En una lectura gruesa, uno podrÃ­a pensar que `bad_data` y `review` comparten la misma patologÃ­a porque ambos pueden tener masa en bins altos. La imagen ayuda a matizar eso: el porcentaje contra `daily` mide violencia contra el Ã¡rbitro diario, pero no distingue aÃºn la causa.
- Si un label concentra mucha masa en el extremo `100`, el conflicto no es una fricciÃ³n perifÃ©rica; es una contradicciÃ³n casi total con la barra diaria. Eso endurece la lectura del bucket, aunque todavÃ­a no cierre su causalidad.
- Si otro label reparte masa entre bins intermedios, la historia cambia: ahÃ­ el problema puede ser de degradaciÃ³n parcial, no de colapso.
- Por eso la figura es Ãºtil no solo por el extremo alto, sino por la **forma completa de la distribuciÃ³n** dentro de cada label.
...
![Outside 1m](images/evidence_assets/global_universe/05_outside_1m_severity_by_label.png)
...
- AquÃ­ la granularidad fina importa mÃ¡s que en `daily`, porque `1m` capta conflictos que la barra diaria puede ocultar.
- La imagen sirve para ver dÃ³nde un label no solo protesta contra el Ã¡rbitro grueso, sino contra la estructura intradÃ­a fina. Eso es especialmente importante para:
...
- En `bad_data`, `outside_1m = 100%` afecta al `30.94%`, pero el bin `(20,100)` aÃ±ade otro `23.25%`. En total, mas de la mitad del bucket sufre conflicto intraminuto severo.
...
- Si un label acumula masa fuerte en bins altos frente a `1m`, ya no estamos viendo solo una discusiÃ³n sobre barra diaria; estamos viendo un choque contra el Ã¡rbitro de mayor resoluciÃ³n.
- La figura tambiÃ©n ayuda a detectar un patrÃ³n Ãºtil: labels que parecen moderados frente a `daily` pueden endurecerse mucho frente a `1m`. Esa divergencia es metodolÃ³gicamente muy informativa.
- En otras palabras, esta imagen no solo aÃ±ade detalle; **cambia la jerarquÃ­a de evidencia** de ciertos buckets.
...
![Duplicacion](images/evidence_assets/global_universe/06_duplicate_severity_by_label.png)

![Odd lots](images/evidence_assets/global_universe/07_odd_lot_severity_by_label.png)
...
- Estas dos imÃ¡genes deben leerse juntas porque separan dos mecanismos que a veces se confunden:
  - textura microestructural legÃ­tima o semi-legÃ­tima;
  - deterioro estructural del tape.
- Si `review_microstructure` apareciera dominado por duplicaciÃ³n dura, serÃ­a un bucket mucho mÃ¡s sospechoso. Lo que se ve, en cambio, es que su identidad real estÃ¡ mucho mÃ¡s cerca de odd-lots y textura.
- Los nÃºmeros lo dejan bastante claro:
  - en `review_microstructure`, `duplicate = 0` todavÃ­a representa `45.69%`
  - `(0,1]%` representa `32.80%`
  - `>10%` apenas `0.26%`
- Es decir, la duplicacion extrema es marginal dentro del bucket. No es su motor principal.
- La cola de duplicaciÃ³n, cuando pesa, desplaza la lectura hacia daÃ±o de integridad y obliga a mirar mÃ¡s allÃ¡ del precio.
- La imagen de odd-lots es especialmente importante porque visualmente demuestra que hay una masa enorme cuya rareza no nace de corrupciÃ³n bruta sino de composiciÃ³n de prints.
...
- Esto no absuelve automÃ¡ticamente esos casos para todos los pipelines, pero sÃ­ cambia su tratamiento institucional: no pueden colapsarse a `bad_data`.
...
![Cobertura 1m](images/evidence_assets/global_universe/08_has_1m_reference_by_label.png)
...
- Esta figura no habla de calidad del tape; habla de **calidad de la decisiÃ³n** que podemos tomar sobre el tape.
- La masa sin `1m` no debe interpretarse como caos. Debe interpretarse como limitaciÃ³n de resoluciÃ³n del Ã¡rbitro.
- Cuantitativamente, casi todo el universo relevante sÃ­ tiene `1m`:
  - `review = 97.07%` con `1m`
  - `reference_scale_mismatch = 99.81%`
  - `review_microstructure = 99.89%`
  - `bad_data = 99.77%`
- Eso significa que la falta de `1m` no es una explicaciÃ³n general del bloque; es una condiciÃ³n muy localizada.
- `review_no_1m_reference` sÃ­ es extremo por definiciÃ³n: `100%` sin `1m`. La imagen prueba que el bucket es real y no una etiqueta teÃ³rica sin base poblacional.
- En `review_no_1m_reference`, la imagen cumple una funciÃ³n muy concreta: demostrar que el bucket no es un invento verbal, sino una consecuencia poblacional de cobertura realmente incompleta.
- TambiÃ©n sirve para no sobreactuar ante ciertos conflictos. Si falta el Ã¡rbitro fino, el cierre del caso debe ser epistemolÃ³gicamente mÃ¡s humilde.
- La imagen ademÃ¡s ayuda a separar dos errores distintos:
  - â€œno tengo suficiente evidencia para absolverâ€
  - â€œtengo evidencia suficiente para condenarâ€
...
![Waterfall rehabilitacion](images/evidence_assets/global_universe/09_review_rehabilitation_waterfall.png)
...
- Esta es probablemente la imagen mÃ¡s importante del bloque entero, porque reordena la pregunta central.
- Sin ella, el inspector puede quedarse atrapado en la barra minÃºscula de `good`. Con ella, entiende que la variable maestra no es `good`, sino la masa rehabilitable dentro de `review`.
- El salto entre:
  - `review_total`
  - `strict recoverable`
  - y `extended recoverable`
  
  enseÃ±a dos cosas a la vez:
  - cuÃ¡nta utilidad real ya estÃ¡ ganada con una policy conservadora;
  - cuÃ¡nta utilidad adicional solo aparece si aflojamos la regla.
- En numeros:
  - `strict recoverable = 68.60%`
  - la ampliacion hasta `extended` solo aÃ±ade `3.66%` del bucket
...
- La distancia entre la versiÃ³n estricta y la extendida no es enorme. Eso es bueno: sugiere que la regla estricta ya captura la parte principal de la masa recuperable.
- La masa que queda fuera sigue siendo grande. Eso tambiÃ©n importa: evita la tentaciÃ³n de declarar victoria prematura sobre `review`.
...
![bad_data subfamilias](images/evidence_assets/global_universe/10_bad_data_visual_subfamilies.png)
...
- La imagen corrige una simplificaciÃ³n peligrosa: pensar que todo `bad_data` es colapso grotesco de precio.
- SÃ­, la subfamilia dominante es `colapso_escala_rango`. Eso confirma que el panel de precio actual cubre muy bien la parte principal del bucket.
...
- Pero la segunda observaciÃ³n fuerte es que existe una masa no trivial de:
  - `integridad_estructural`
  - `mixto_estructural_rango`
  - `conflicto_ralo_o_sparse`
- Esto cambia la obligaciÃ³n visual del dossier. Ya no basta un Ãºnico panel â€œbonitoâ€ de precio para representar todo `bad_data`.
- La ausencia de `conflicto_rango_local` en este cierre concreto tambiÃ©n es informativa: significa que, bajo la taxonomÃ­a actual, los rechazos duros locales tienden a quedar absorbidos por otras subfamilias mÃ¡s fuertes.
...
![review microstructure texturas](images/evidence_assets/global_universe/11_review_microstructure_textures.png)
...
- Esta figura es muy fuerte porque destruye una lectura equÃ­voca del bucket. `review_microstructure` no es, poblacionalmente, una colecciÃ³n heterogÃ©nea de rarezas pequeÃ±as; estÃ¡ dominado de forma masiva por una firma muy concreta.
- La dominancia extrema de `odd_lot_dominante` significa que, a nivel de poblaciÃ³n, este bucket estÃ¡ mucho mÃ¡s cerca de una cuestiÃ³n de composiciÃ³n de prints que de una cuestiÃ³n de â€œtape rotoâ€.
...
- Las colas de `duplicacion_textura`, `conflicto_fino_1m` y `sparse_o_ralo` existen, pero no son la identidad central del bucket. Son subproblemas, no el corazÃ³n de la familia.
- Esto obliga a que la documentaciÃ³n por casos no sobrerrepresente ejemplos raros de duplicaciÃ³n si luego el universo real estÃ¡ controlado por odd-lots.
- TambiÃ©n cambia el orden de prioridades: si queremos mejorar polÃ­tica o visualizaciÃ³n de esta familia, primero hay que explicar mejor odd-lots, no empezar por la cola marginal.
...
![reference scale buckets](images/evidence_assets/global_universe/12_reference_scale_mismatch_buckets.png)
...
- Esta figura prueba que `reference_scale_mismatch` no es simplemente el bucket de un Ãºnico split mal normalizado o de una Ãºnica relaciÃ³n `10x`.
- El peso grande de `>1x_other` es muy importante: sugiere que el conflicto de escala es amplio y menos â€œlimpioâ€ de lo que serÃ­a deseable para una reconciliaciÃ³n trivial.
- En concreto:
  - `>1x_other = 33.59%`
  - `~10x = 14.47%`
  - `~20x = 8.14%`
  - `~1x = 5.81%`
  - `~15x = 5.63%`
- Sumando solo los cuatro buckets visibles principales no llegamos ni al `63%`. Eso significa que el bucket sigue muy fragmentado incluso despuÃ©s de sacar sus grandes masas.
- La coexistencia de `~10x`, `~20x`, `~15x`, `~0.5x`, `~4x`, `~5x` muestra un paisaje heterogÃ©neo. Eso refuerza que este bucket necesita una metodologÃ­a de reconciliaciÃ³n, no una receta Ãºnica.
- La presencia visible de `~1x` dentro del propio bucket es otra seÃ±al Ãºtil: indica que el cierre a `reference_scale_mismatch` no depende solo de un ratio brutal, sino del conjunto de la evidencia.
- En otras palabras, el bucket no es â€œpatolÃ³gico de una sola maneraâ€; es una familia ancha de desacoples de escala.
...
![review rehab detalle](images/evidence_assets/global_universe/13_review_rehabilitation_categories.png)
...
- La figura obliga a dejar de pensar en `review` como un residuo homogÃ©neo. Su estructura interna estÃ¡ claramente estratificada.
- La masa dominante en `strict_recoverable` prueba que el bucket no es un pantano sin salida; una gran parte ya estÃ¡ esencialmente ganada bajo policy conservadora.
- En numeros:
  - `strict_recoverable = 68.60%`
  - `extended_only = 3.66%`
  - `near_1x_but_not_recoverable = 24.56%`
  - `not_near_1x = 3.19%`
- La lectura estratÃ©gica nace justo de esa comparaciÃ³n:
  - la masa realmente problemÃ¡tica no es la mÃ¡s lejana (`3.19%`);
  - es la enorme franja que ya estÃ¡ cerca de `1x` pero aÃºn no cruza la policy (`24.56%`).
- La franja `extended_only` es pequeÃ±a frente al total. Eso sugiere que la ganancia por aflojar la policy existe, pero no redefine el bloque.
- La franja verdaderamente estratÃ©gica es `near_1x_but_not_recoverable`: es enorme y estÃ¡ cerca del umbral correcto. Esa es la masa donde una mejor comprensiÃ³n puede cambiar mÃ¡s decisiones reales.
- `not_near_1x` es mucho menor; eso importa porque indica que el residuo mÃ¡s lejano existe, pero no es la mayor frontera de trabajo.
- La imagen, por tanto, no solo describe `review`; establece una **prioridad de investigaciÃ³n**.
