# 002_DAILY

Esta carpeta es una copia local navegable del dossier de auditoria `daily`.

## Origen

Fuente canonica original:

`C:\TSIS_Data\01_TSIS_DATA_FOUNDATION\01_foundations\inspection_dossiers\daily`

La copia se preparo en:

`C:\TSIS_Data\00_CTO_1\001_DATA_local_audit\002_DAILY`

Fecha de reparacion de enlaces: 2026-07-11.

## Estructura local

- `daily_inspection_readout_v0_1.md`: lectura institucional/cierre de la inspeccion `daily`.
- `evidence/`: markdown de casos copiados previamente en esta carpeta.
- `images/`: copia local de las imagenes referenciadas por los markdown. La estructura interna replica la ruta relativa dentro del dossier original.
- `referenced_docs/`: copia local de markdown auxiliares enlazados desde los documentos principales cuando no existian ya en `evidence/`.

## Reparacion aplicada

Los enlaces de imagen se reescribieron para apuntar a `images/` con rutas relativas desde cada markdown. Los enlaces a markdown auxiliares se reescribieron hacia copias locales en `referenced_docs/` o hacia los markdown ya existentes en `evidence/`.

## Fuente de verdad

Esta carpeta es una copia operativa para lectura y navegacion. La fuente institucional de verdad sigue siendo el dossier original bajo `01_foundations/inspection_dossiers/daily`.
