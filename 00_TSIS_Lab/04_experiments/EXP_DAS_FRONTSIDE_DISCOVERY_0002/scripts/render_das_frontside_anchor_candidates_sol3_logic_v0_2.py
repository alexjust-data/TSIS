...
ANCHORS_DEFAULT = RUN_ROOT_DEFAULT / "anchor_candidates_sol3_logic_v0_2" / "das_frontside_anchor_candidates_sol3_logic_v0_2.parquet"
OUTPUT_DEFAULT = RUN_ROOT_DEFAULT / "visual_audit_png_sol3_logic_v0_2"

ET = ZoneInfo("America/New_York")
RENDERER_VERSION = "render_das_frontside_anchor_candidates_sol3_logic_v0_2_scanner_gate"
...
def _load_session_bars(path: Path, ticker: str, session_date: str) -> pd.DataFrame:
    ...
    raw = raw[(raw["time_et"] >= "03:30") & (raw["time_et"] <= "10:00")].copy()
    if raw.empty:
        raise ValueError(f"no 03:30-10:00 bars for {ticker} {session_date}: {path}")
    ...
    cum_vol = out["v"].cumsum().replace(0, np.nan)
    out["vwap"] = ((typical * out["v"]).cumsum() / cum_vol).ffill()
    return out
...
def _build_anchors(row: pd.Series) -> list[dict[str, Any]]:
    ...
    if fake:
        anchors.append(fake)
    if rebreak and state == "rebreak_confirmed":
...
def _draw_ema_wilder_regime_lines(
    ...
    for i in range(len(df) - 1):
        row0 = df.iloc[i]
        row1 = df.iloc[i + 1]
        vals = [row0["ema8"], row0["wilder8"], row1["ema8"], row1["wilder8"]]
        if any(pd.isna(v) for v in vals):
            continue

        ema0 = float(row0["ema8"])
        ema1 = float(row1["ema8"])
        wild0 = float(row0["wilder8"])
        wild1 = float(row1["wilder8"])
        if not all(math.isfinite(v) for v in [ema0, ema1, wild0, wild1]):
            continue

        # Regla visual DAS: Wilder por encima de EMA = regimen rojo; por debajo = regimen verde.
        regime_color = RED_DARK if ((wild0 + wild1) / 2.0) > ((ema0 + ema1) / 2.0) else GREEN_DARK
        x0 = x_for_idx(i)
        x1 = x_for_idx(i + 1)
        draw.line((x0, y_for_price(ema0), x1, y_for_price(ema1)), fill=regime_color, width=ema_width)
        draw.line((x0, y_for_price(wild0), x1, y_for_price(wild1)), fill=regime_color, width=wilder_width)
...
def _render_case(row: pd.Series, case_ordinal: int, output_dir: Path, overwrite: bool = False) -> dict[str, Any]:
    ...
    LEFT, RIGHT = 78, 1880
    TITLE_Y = 46
    PRICE_TOP, PRICE_BOTTOM = 380, 1212
    VOL_TOP, VOL_BOTTOM = 1252, 1548
    AUDIT_BOX_TOP = 145
    AUDIT_GAP_TO_CHART = 28
    ...
    subtitle = (
        f"event-day 03:30-10:00 NY detail | state={row.get('trajectory_state')} | "
    ...
    for i in range(len(df) - 1):
        ...
        if pd.isna(row0["ema8"]) or pd.isna(row0["wilder8"]) or pd.isna(row1["ema8"]) or pd.isna(row1["wilder8"]):
            continue
        ema_avg = (float(row0["ema8"]) + float(row1["ema8"])) / 2.0
        wilder_avg = (float(row0["wilder8"]) + float(row1["wilder8"])) / 2.0
        color = RED if wilder_avg > ema_avg else GREEN
        poly = [
    ...
    _draw_ema_wilder_regime_lines(draw, df, x_for_idx, y_for_price, ema_width=3, wilder_width=4)
    ...
    # Label boxes: fixed audit band under the title/subtitle, detached from prior close.
    # This keeps prior close readable and prevents labels from hiding candles.
    box_order = ["first_push_high", "first_dip_low", "fake_rebreak", "rebreak_confirmed"]
    boxes = []
    ...
    if boxes:
        ...
        top_y = AUDIT_BOX_TOP
        if top_y + max_h > PRICE_TOP - AUDIT_GAP_TO_CHART:
            top_y = max(TITLE_Y + 92, PRICE_TOP - AUDIT_GAP_TO_CHART - max_h)
        x = start_x
    ...
    return {
        "visual_inspection_manifest_id": "visual_inspection_manifest_v0_3",
        ...
        "indicator_regime_policy": "wilder8_above_ema8_red_else_green; ema8_width=3; wilder8_width=4",
...
def main() -> int:
    ...
    manifest_csv = output_dir / "visual_inspection_manifest_v0_3.csv"
    manifest_parquet = output_dir / "visual_inspection_manifest_v0_3.parquet"
    manifest_json = output_dir / "visual_inspection_manifest_summary_v0_3.json"
