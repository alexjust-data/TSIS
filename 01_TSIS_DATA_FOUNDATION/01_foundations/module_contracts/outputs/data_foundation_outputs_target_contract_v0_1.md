...
`D:/quotes -> E:/TSIS/data/quotes_` is closed and approved.
The official quotes E-root for new downstream work is `E:/TSIS/data/quotes_`,
backed by Phase A structural parity and Phase B SHA256 retry evidence.

Historical artifacts built from `D:/quotes` remain pre-approval/provenance evidence and must be
rebuilt against the approved E-root before promotion to an official downstream table.
The legacy `E:/TSIS/data/quotes` tree remains incomplete for this decision.
...
La autoridad primaria sigue viviendo en este contrato, en la status matrix y en
el leaf Graphify original:
...
La carpeta `C:/TSIS_Data/00_CTO_1/003_STADE_tables/` no es una source of truth
independiente. Si se modifica informacion material en los originales, la copia
secundaria debe actualizarse en la misma operacion para mantener ambas
superficies emparejadas.
...
C:/TSIS_Data/01_TSIS_DATA_FOUNDATION/01_foundations/graphify-out/_graphify_staging_foundations_authority_20260620/graphify-out/graph.json
```
...
| `intraday_scanner_candidates_table` | Candidatos in-play detectados desde `ohlcv_1m`: primer cruce +50%, segmento premarket/regular/afterhours, volumen-to-time, dollar-volume-to-time, max move y lineage. | Reconstruir cuando un ticker se puso realmente in-play durante extended hours. Para estrategias intradia sustituye al proxy diario como detector de primer push; no es estado completo ni senal. |
...
C:/TSIS_Data/01_TSIS_DATA_FOUNDATION/data/reference/market_calendar_official_XNYS_20050101_20260309.meta.json
C:/TSIS_Data/01_TSIS_DATA_FOUNDATION/scripts/agent05_build_market_calendar_official.py
...
```text
builder: C:/TSIS_Data/01_TSIS_DATA_FOUNDATION/scripts/agent05_build_market_calendar_official.py
...
5e423e444e1228a671a05159eda0a707f9bb2446f5b17860740f3001edbbd954
...
last_session: 2026-03-09
early_close_sessions: 45
build_run_id: market_calendar_v0_1_20260630T193931Z
source_parquet_sha256: 5e423e444e1228a671a05159eda0a707f9bb2446f5b17860740f3001edbbd954
output_sha256: cbf1879261866d980c5a8542fadf683dbc91f96b80b7865055417a16d1e6e87c
hard_fail_count: 0
...
rows_per_family: 7369699
tickers: 4824
first_session: 2005-01-03
last_session: 2026-03-09
parquet_file_count: 88
tree_sha256: c8ccf548976920ea5e4d6b666ce6f70a009bc98da63e23a2f29a83e7e7cfd500
build_run_id: expected_data_calendar_v0_1_20260630T194807Z
hard_fail_count: 0
duplicate_key_groups: 0
invalid_window_count: 0
test_evidence: pytest 2026-07-01 targeted market_calendar_expected_data_calendar_master_daily_contracts
```
...
- separa denominador elegible, denominador in-play momentum y visibilidad
  operativa humana;
...
Modelo v0.3 vigente para nuevos builders:
...
in_play_momentum_candidate_denominator_v0_3
  -> base eligible + movimiento fuerte >= 50% + volumen/tradability minimo

trade_station_like_profile_v0_3
  -> visibilidad operativa humana: volume_today >= 500000 + pct_chg_1d top 25

strategy overlays
  -> DAS y cualquier estrategia futura se aplican despues del scanner global
```

En v0.3, `volume_today >= 500000` no es por si solo el scanner global. Forma
parte de la tradability gate junto con `dollar_volume_today >= 250000`, y sigue
existiendo como filtro del perfil de visibilidad tipo TradeStation.

La deteccion de movimiento fuerte usa un proxy diario controlado:
...
Esto permite capturar pump-and-dump/frontside que hizo high intradia aunque
cierre por debajo. No certifica todavia si el primer push ocurrio en premarket,
regular session o after-hours. Para esa semantica debe usarse
`intraday_scanner_candidates_table_v0_1` o una version intradia posterior.

La tabla puede llevar perfiles de visibilidad, pero estos no deben leerse como
estrategias ni como embudo secuencial. DAS y cualquier otra estrategia futura
deben tener overlay/contrato propio posterior.
...
### 5B. `intraday_scanner_candidates_table`

Clase:

```text
intraday candidate generation table
```
...
- reconstruye el primer cruce intradia de +50% vs `prior_close`;
- conserva si el primer cruce fue `premarket`, `regular` o `afterhours`;
- conserva `volume_to_time_at_first_cross` y
  `dollar_volume_to_time_at_first_cross`;
- permite sembrar overlays de estrategia y futuros builders de
  `market_state_table` / `event_state_table`;
- sustituye al proxy diario cuando el timing del push importa.

Regla central:

```text
daily_scanner_candidates_table_v0_3 = daily/EOD coarse context
intraday_scanner_candidates_table_v0_1 = first-push timing candidate surface
```

No debe contener:

- microestructura quotes/trades completa;
- labels, outcomes, rewards, fills, PnL o decision de estrategia;
- float como filtro hasta que exista `float_context_table`;
- afirmacion de full universe salvo materializacion 20y con manifest oficial.
...
Estado ejecutado adicional:

```text
master_intraday_bar_table_v0_2_candidate_quote_guarded = scoped_candidate_materialized_not_official
...
La ruta tiene dos artefactos distintos y no deben mezclarse:
...
1. repair_manifest_lt1b_v0_1.parquet
   ...
   role = manifest/overlay oficial de repairs quote-guarded

2. ohlcv_1m_quote_guarded_full_universe_v0_1
   physical_root = C:/TSIS_Data/data/data_foundation_outputs/ohlcv_1m_quote_guarded_full_universe_v0_1
   ...
   status_from_builder_readme = candidate_not_official
   full_universe_claim = false
   role = arbol fisico derivado candidate de barras 1m quote-guarded
```

Estado verificado contra metadata local:

```text
latest_run_id = qg_1m_full_2005_2014_parallel_v0_1_20260707T113327Z
build_runs_root = C:/TSIS_Data/data/data_foundation_outputs/ohlcv_1m_quote_guarded_full_universe_v0_1/_build_runs

physical_materialization_present = true
years_present = 2005..2026
global_clean_close_2005_2026 = false

2005..2014: final_status=complete, failed_tickers=0.
2015..2020: final_status=complete_with_failures, failed_tickers > 0.
2021..2024: final_status=complete, failed_tickers=0.
...
```text
No decir que no existe arbol fisico quote-guarded: existe como candidate bajo
C:/TSIS_Data/data/data_foundation_outputs/ohlcv_1m_quote_guarded_full_universe_v0_1.

No decir que esta full terminado limpio 2005..2026: la metadata local conserva
complete_with_failures / failed_tickers > 0 para 2015..2020.

No decir que es dataset oficial/promoted full-universe: el README del builder lo
marca como candidate_not_official y full_universe_claim=false. Antes de tratarlo
como institucional/promovido falta cierre de failures 2015..2020, reconciliacion
global, validacion anual/global, inspeccion adversarial y cierre de lineage.

El raw canonical E:/TSIS/data/ohlcv_1m no se modifica. La salida fisica candidate
se deriva desde raw ohlcv_1m + repair_manifest_lt1b_v0_1 + repair shards.
```
...
```text
seed input:
C:/TSIS_Data/01_TSIS_DATA_FOUNDATION/configs/data_foundation_outputs/microstructure_features_seed_windows_v0_1.csv
...
source_windows_csv: C:/TSIS_Data/01_TSIS_DATA_FOUNDATION/runs/data_foundation/microstructure_features_table_v0_2_candidate_controlled_25_per_role/microstructure_features_table_v0_2_candidate_window_manifest_v0_1.csv
...
Contrato de ensamblaje del state builder:

```text
01_foundations/module_contracts/outputs/state_builder_contract_v0_1.md
```

Contrato de tablas candidatas de eventos daily/1m:
...
- contratos de eligibility, formula, timestamp, role y builder versionados.
...
state builder contract: defined
observable eligibility contract: complete_for_declared_scope
derived formula contract: complete_for_declared_scope
decision timestamp policy: complete_for_declared_scope
snapshot roles contract: complete_for_declared_scope
...
```text
1. daily_scanner_candidates_table v0.3 controlled historical replay
...
5. daily_strategy_candidate_events_table controlled replay materialization DONE; decidir si se rerun wider/E-root daily
6. daily_strategy_event_windows controlled candidate DONE desde los 69 eventos daily
7. ejecutar validate_event_candidate_tables.py sobre cualquier daily candidate wider/E-root nuevo
8. ejecutar materialize_strategy_candidate_events_table.py sobre intraday quote-guarded candidate real DONE controlled via adapter desde master_intraday
9. ejecutar validate_event_candidate_tables.py sobre intraday candidate real DONE controlled
10. expansion wider de event_windows para eventos daily/1m no-halt
11. RAW-to-consumption lineage daily event windows controlled DONE; intradia 1m quote-guarded upstream manifest lineage DONE, preflight ligero passed, muestra controlada passed_not_official, scoped candidate passed_not_official y E-root scoped candidate passed_not_official; completar micro/contexto si entran en state fixture
12. sample real controlado de market_state_table
13. sample real controlado de event_state_table
14. short_sale_constraints_table despues de adquirir fuentes SSR/borrow/locate
15. real_time_corporate_event_alerts_table despues del contrato de latencia live/vendor
```
...
daily_strategy_candidate_events_table_controlled_materialization = passed_69_rows
daily_strategy_event_windows_controlled_candidate = passed_207_rows
```

Evidencia controlada de event windows daily:

```text
C:/TSIS_Data/tests/test_runs/2026-07-05/daily_strategy_event_windows_from_69_daily_events_controlled/
event_windows_table_v0_1_candidate_daily_strategy_events/data.parquet
source_event_count = 69
row_count = 207
validator_status = passed
```

Esto no valida una tabla real amplia/E-root todavia. El builder y el validator ya pasan fixture-scope; existe evidencia controlada daily, event_windows controlado e intradia quote-guarded controlado. Faltan los scopes wider/E-root daily e intradia full-universe y conservar manifest/summary/validator output para esos runs.

Precondicion de trazabilidad RAW -> consumo cerrada para el scope declarado:
...
Lectura correcta: antes de consumir cualquier componente en `market_state_table` o `event_state_table`, debe existir una seccion `Trazabilidad RAW -> Consumo De Estado` que muestre fuente RAW/staged, derivadas intermedias, builders, manifests, transformaciones, cutoff, quality gates, consumo permitido y gaps. El primer ejemplo cerrado es `master_daily_table_v0_1`.

Lineage controlado del Camino A daily cerrado:

```text
01_foundations/module_contracts/outputs/state_raw_to_consumption_lineage_daily_event_windows_controlled_v0_1.md
state_raw_to_consumption_lineage_daily_event_windows_controlled_v0_1 = complete_for_controlled_scope
```

Cubre master_daily -> daily scanner v0.3 -> daily event candidates -> event_windows controladas. No cubre intradia 1m ni materializacion oficial E-root.

Lineage intradia 1m quote-guarded cerrado a nivel upstream manifest:

```text
01_foundations/module_contracts/outputs/state_raw_to_consumption_lineage_intraday_1m_quote_guarded_v0_1.md
state_raw_to_consumption_lineage_intraday_1m_quote_guarded_v0_1 = complete_for_upstream_manifest_lineage_scope
```

Cubre raw `E:/TSIS/data/ohlcv_1m` + `D:/quotes` provisional -> repair shards -> `repair_manifest_lt1b_v0_1.parquet` PASS -> vista quote-guarded -> `master_intraday_bar_table_v0_2_candidate_quote_guarded` scoped E-root -> market_state intradia controlado -> intraday 1m strategy candidate events controlados. No materializa scanner 1m v0_2 completo, event_state, outputs oficiales/full-universe, ML/RL ni AlphaEvolve. Event_windows intradia ya existe como candidato controlado/no oficial.

Preflight ligero intradia 1m quote-guarded ejecutado:

```text
script = scripts/preflight_master_intraday_quote_guarded_candidate.py
report = C:/TSIS_Data/tests/test_runs/2026-07-05/master_intraday_quote_guarded_candidate_preflight_v0_1/master_intraday_quote_guarded_candidate_preflight_v0_1.json
validator_status = passed
...
Materializacion controlada de muestra intradia 1m quote-guarded ejecutada:

```text
script = scripts/materialize_master_intraday_quote_guarded_candidate_sample.py
manifest = C:/TSIS_Data/tests/test_runs/2026-07-05/master_intraday_quote_guarded_candidate_sample_v0_1/_master_intraday_bar_table_v0_2_candidate_quote_guarded_controlled_sample_manifest.json
...
Materializacion scoped candidate intradia 1m quote-guarded ejecutada:

```text
script = scripts/materialize_master_intraday_quote_guarded_candidate_scoped.py
manifest = C:/TSIS_Data/tests/test_runs/2026-07-05/master_intraday_quote_guarded_candidate_scoped_v0_1/_master_intraday_bar_table_v0_2_candidate_quote_guarded_scoped_manifest.json
status = scoped_candidate_materialized_not_official
...
E-root scoped candidate intradia 1m quote-guarded materializado:

```text
manifest = E:/TSIS/data/data_foundation_outputs/master_intraday_bar_table/_master_intraday_bar_table_v0_2_candidate_quote_guarded_manifest.json
output = E:/TSIS/data/data_foundation_outputs/master_intraday_bar_table/master_intraday_bar_table_v0_2_candidate_quote_guarded/data.parquet
status = scoped_candidate_materialized_not_official
...
Siguiente paso permitido: decidir scope wider/declared-universe o consumir este scoped candidate en un fixture de estado controlado, sin full-universe claim ni consumo ML/RL/AlphaEvolve.
...
```text
Canonical State = tablero estable, legal y gobernado.
Representation Layer = espacio de busqueda para scores, embeddings, parameter grids,
semantic states, detectores, transiciones y politicas candidatas.
```

Este contrato no cambia el orden materializador anterior. Solo obliga a que los
futuros validators/builders no mezclen representaciones experimentales dentro de
`market_state_table` o `event_state_table` como verdad canonica.
...
- `daily_scanner_candidates_table`: primera candidata de este loop porque
  ...
  salvo denominador probado. El camino activo es v0.3: primero se materializa
  `base_eligible_smallcap_denominator_v0_3`, despues
  `in_play_momentum_candidate_denominator_v0_3`. El movimiento fuerte inicial
  es `>=50%` y la tradability gate inicial es `volume_today >= 500000` OR
  `dollar_volume_today >= 250000`. El replay diario v0.3 usa proxy EOD
  (`daily_high_vs_prev_close_pct`, `pct_chg_1d`, `gap_pct`) y no certifica
  segmento premarket/regular/afterhours. El contrato objetivo activo vive en
  `01_foundations/module_contracts/outputs/daily_scanner_candidates_table_target_contract_v0_3.md`.
  El contrato de definiciones activo vive en
  `01_foundations/module_contracts/outputs/scanner_framework_and_definitions_contract_v0_3.md`.
  Las ejecuciones largas/multi-year deben pasar por
  `scripts/run_daily_scanner_candidates_materialization_v0_3.ps1`, que escribe
  pre-manifest, heartbeat, PID manifest, logs y `_run_summary.json` por ventanas
  anuales. No se debe lanzar el builder Python directo para un run de 20 anos.
...
Controlled intraday 1m strategy candidate events quote-guarded:
...
Lectura correcta: este run controla la ruta 1m quote-guarded hacia eventos candidatos. No es estado base, no contiene outcomes y no esta promovido.

Controlled intraday 1m strategy event windows:
...
Lectura correcta: ventanas intradia controladas listas para `event_state` fixture; no outcomes inline.

Controlled intraday 1m event_state:

```text
script = scripts/materialize_event_state_intraday_quote_guarded_candidate.py
...
Lectura correcta: event_state intradia controlado listo para outcomes separados; no outcomes inline y no promocion oficial.

## 2026-07-05 Addendum - Controlled Intraday 1m Outcomes Candidate

Este addendum no cambia `outcomes_table_v0_1` oficial, que sigue siendo daily
next-session para halts bajo su scope declarado. Registra el output candidate
controlado que cierra la ruta intradia 1m quote-guarded hasta `y` separado.
...
```text
script = scripts/materialize_intraday_1m_event_outcomes_candidate.py
test = tests/data_foundation_outputs/test_intraday_1m_event_outcomes_candidate_builder.py
...
```text
outcomes intradia = y separado
market_state/event_state = X observable bajo cutoff
MFE/MAE/returns post-evento no pueden entrar inline en estado
labels ML y rewards RL requieren contratos posteriores
```
...
-> labels/rewards solo si existe contrato especifico
