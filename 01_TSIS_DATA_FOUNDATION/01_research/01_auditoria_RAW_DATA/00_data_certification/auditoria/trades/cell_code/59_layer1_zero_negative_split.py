...
TARGET_FILES_TOTAL = 167_606
ARTIFACT_DIR = Path(__file__).resolve().parent / "59_layer1_zero_negative_split_artifacts"
...
def _classify_file_sizes(file_path: str | Path) -> dict:
    x = pd.read_parquet(Path(file_path), columns=["size"])
    s = pd.to_numeric(x["size"], errors="coerce").dropna()
...
def build_layer1_zero_negative_scan(
    ...
    for batch in handle.stream(columns=cols, batch_size=batch_size, normalize=True):
        if "m.negative_or_zero_size_rows" not in batch.columns:
            continue

        sub = batch.loc[pd.to_numeric(batch["m.negative_or_zero_size_rows"], errors="coerce").fillna(0) > 0].copy()
        if sub.empty:
            continue

        for row in sub.itertuples(index=False):
            info = _classify_file_sizes(row.file)
            rows.append(
                {
                    "file": row.file,
...
def _artifact_paths() -> dict[str, Path]:
    return {
        "scan": ARTIFACT_DIR / "layer1_zero_negative_scan.parquet",
        "negative_values": ARTIFACT_DIR / "layer1_negative_values.parquet",
        "summary": ARTIFACT_DIR / "layer1_zero_negative_summary.parquet",
        "manifest": ARTIFACT_DIR / "manifest.json",
    }


def persist_layer1_zero_negative_artifacts(scan_df: pd.DataFrame, negative_values: np.ndarray) -> dict[str, Path]:
...
def plot_zero_negative_pie(scan_df: pd.DataFrame, figsize=(4.2, 4.2)) -> None:
    ...
    ax.set_title(f"Files con size <= 0 | total={int(counts.sum()):,}")
    plt.tight_layout()
    plt.show()
...
def plot_negative_distribution(negative_values: np.ndarray) -> None:
    if len(negative_values) == 0:
        display(Markdown("No hay valores `size < 0` en estos files. La masa de `167,606` se explica por ceros."))
        return
    ...
    axes[0].set_title("Distribución de size negativos")
    axes[0].set_xlabel("size < 0")
    ...
    axes[1].set_xlabel("|size| negativo")
...
def run_layer1_zero_negative_split(
    head_rows: int = 15,
    batch_size: int = 50_000,
    max_files: int | None = None,
) -> tuple[pd.DataFrame, np.ndarray]:
    scan_df, negative_values = build_layer1_zero_negative_scan(
        batch_size=batch_size,
        max_files=max_files,
    )
    artifact_paths = persist_layer1_zero_negative_artifacts(scan_df, negative_values)

    total = len(scan_df)
    display(Markdown(
        f"**Separación global de files con `size <= 0`**  \n"
    ...
    display(Markdown(f"**Ejemplos `size == 0`** `head({head_rows})`"))
    display(zero_examples)
    display(Markdown(f"**Ejemplos `size < 0`** `head({head_rows})`"))
    display(neg_examples)
    display(Markdown(f"**Ejemplos `size == 0 y size < 0`** `head({head_rows})`"))
    ...
    if len(negative_values):
        ...
        display(Markdown("**Resumen de negatividad real (`size < 0`)**"))
        display(neg_stats)
