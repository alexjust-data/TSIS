...
def build_time_concentration_chunked(current_parquet: str, top_n: int = 25):
    ...
    for row_group_idx in range(pf.num_row_groups):
        chunk = pf.read_row_group(
            row_group_idx,
            columns=["severity", "ticker", "date"],
        ).to_pandas()
        if chunk.empty:
            continue

        date_ts = pd.to_datetime(chunk["date"], errors="coerce")
        chunk["month"] = date_ts.dt.to_period("M").astype(str)
        chunk["year"] = date_ts.dt.year

        month_frames.append(
            chunk.groupby(["month", "severity"], dropna=False)
            .size()
            .rename("files")
            .reset_index()
        )
        year_frames.append(
            chunk.groupby(["year", "severity"], dropna=False)
            .size()
            .rename("files")
            .reset_index()
        )
        ticker_frames.append(
            chunk.groupby(["ticker", "severity"], dropna=False)
            .size()
            .rename("files")
            .reset_index()
        )
    ...
    month_mix_full = (
        ...
        .groupby(["month", "severity"], dropna=False, as_index=False)["files"]
        .sum()
        if month_frames
        else pd.DataFrame(columns=["month", "severity", "files"])
    )
    month_pivot_full = (
        month_mix_full.pivot(index="month", columns="severity", values="files")
        .fillna(0)
    ...
    year_mix_full = (
        ...
        .groupby(["year", "severity"], dropna=False, as_index=False)["files"]
        .sum()
        if year_frames
    ...
    ticker_mix_full = (
        ...
        .groupby(["ticker", "severity"], dropna=False, as_index=False)["files"]
        .sum()
        if ticker_frames
    ...
    ticker_focus_full = ticker_pivot_full.sort_values(
        ["hard_fail_rate_pct", "total"],
        ascending=[False, False],
    ).head(top_n)

    return month_mix_full, month_pivot_full, month_rate_full, year_mix_full, year_pivot_full, year_rate_full, ticker_focus_full
