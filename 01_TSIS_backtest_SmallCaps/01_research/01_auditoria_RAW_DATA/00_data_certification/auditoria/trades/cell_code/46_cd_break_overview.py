...
def build_break_cache_chunk(matched_chunk, target_issue: str = "trade_price_outside_daily_range"):
    daily_break_chunk = build_daily_break_full(matched_chunk, target_issue=target_issue)
    keep_cols = [
        "file", "ticker", "date", "severity", "batch_id", "issues_list", "warns_list",
        "m.trade_vwap", "m.vw", "m.possible_price_scale_factor_vs_daily",
        "m.possible_price_scale_factor_vs_1m", "m.trade_volume_vs_daily_ratio",
        "m.trade_volume_vs_1m_ratio", "m.duplicate_excess_ratio_pct",
        "daily_low", "daily_high", "trade_min", "trade_max", "daily_span",
        "break_below_abs", "break_above_abs", "break_below_pct_span",
        "break_above_pct_span", "break_side", "break_abs_max", "break_pct_span_max",
    ]
    keep_cols = [c for c in keep_cols if c in daily_break_chunk.columns]
    return daily_break_chunk[keep_cols].copy()
...
def run_full_break_overview_chunked(
    current_parquet: str | Path,
    mod00: dict,
    target_issue: str = "trade_price_outside_daily_range",
    top_n: int = 50,
):
    pf = pq.ParquetFile(Path(current_parquet))
    frames: list[pd.DataFrame] = []
    columns = ["file", "ticker", "date", "severity", "batch_id", "issues", "warns", "metrics_json"]

    for row_group_idx in range(pf.num_row_groups):
        chunk = pf.read_row_group(row_group_idx, columns=columns).to_pandas()
        if chunk.empty:
            continue
        chunk = mod00["normalize_event_like_df"](chunk)
        matched = chunk.loc[
            chunk["issues_list"].map(lambda xs: target_issue in set(xs)),
            :,
        ]
        if matched.empty:
            continue
        frames.append(build_break_cache_chunk(matched, target_issue=target_issue))

    daily_break_full = pd.concat(frames, ignore_index=True) if frames else pd.DataFrame()
    display_daily_break_summary(daily_break_full)
    plot_daily_break_overview(daily_break_full)
    plot_daily_break_temporal(daily_break_full)
    top_breaks_head, top_breaks = build_top_breaks(daily_break_full, top_n=top_n)
