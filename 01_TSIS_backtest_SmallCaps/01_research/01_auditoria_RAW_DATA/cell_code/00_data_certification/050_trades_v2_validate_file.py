        ...
        if valid_df.empty:
            ...
            if not daily_ref.get("ohlcv_daily_found", False):
                ...
                if pd.notna(daily_vol) and float(daily_vol) > 0:
                    ...
                    metrics["trade_volume_vs_daily_ratio"] = json_ready(possible_volume_scale_factor)
                ...
                if outside_daily:
                    price_scale_match = (
                        near(possible_price_scale_factor, 0.1)
                        or near(possible_price_scale_factor, 10.0)
                    )
                    volume_scale_match = (
                        near(possible_volume_scale_factor, 0.1)
                        or near(possible_volume_scale_factor, 10.0)
                    )
                    if price_scale_match and volume_scale_match:
            ...
            if not m1_ref.get("ohlcv_1m_found", False):
                ...
                if pd.notna(low_1m) and pd.notna(high_1m):
                    if float(valid_df["price"].min()) < float(low_1m) or float(valid_df["price"].max()) > float(high_1m):
                        if near(possible_price_scale_factor_vs_1m, 0.1) or near(possible_price_scale_factor_vs_1m, 10.0):
                ...
                if pd.notna(vol_1m) and float(vol_1m) > 0:
                    metrics["trade_volume_vs_1m_ratio"] = json_ready(float(valid_df["size"].sum()) / float(vol_1m))
            ...
            if "trade_price_outside_daily_range" in issues:
                daily_scale_match = near(metrics.get("possible_price_scale_factor_vs_daily"), 0.1) or near(metrics.get("possible_price_scale_factor_vs_daily"), 10.0)
                m1_scale_match = near(metrics.get("possible_price_scale_factor_vs_1m"), 0.1) or near(metrics.get("possible_price_scale_factor_vs_1m"), 10.0)
                if daily_scale_match and m1_scale_match:
