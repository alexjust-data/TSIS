...
def factor_near_one(value: float | None, tol_pct: float = 20.0) -> bool:
    if value is None or pd.isna(value):
        return False
    value = float(value)
    if value <= 0:
        return False
    return abs(value - 1.0) / 1.0 * 100.0 <= tol_pct
    ...
    *,
    ...
    if not missing_required_cols:
        ...
        if valid_df.empty:
            ...
            metrics["outside_daily_detected"] = outside_daily
            metrics["outside_1m_detected"] = outside_1m
            ...
            same_scale_context = factor_near_one(possible_price_scale_factor, tol_pct=20.0) and factor_near_one(
                possible_price_scale_factor_vs_1m, tol_pct=20.0
            )
            metrics["same_scale_context"] = same_scale_context

            if same_scale_context and outside_daily and not outside_1m:
                if "trade_price_outside_daily_range" in issues:
                    issues = [x for x in issues if x != "trade_price_outside_daily_range"]
                warns.append("daily_only_range_break_same_scale")

            if same_scale_context and outside_daily and outside_1m:
                if "trade_price_outside_daily_range" in issues:
                    issues = [x for x in issues if x != "trade_price_outside_daily_range"]
                issues.append("trade_price_outside_daily_and_1m_range")
