...
def deterministic_quotes_files(root: Path):
    for dirpath, dirnames, filenames in os.walk(root):
        dirnames.sort()
        filenames.sort()
        if "quotes.parquet" not in filenames:
            continue
        yield Path(dirpath) / "quotes.parquet"


def build_row(root: Path, label: str, p: Path) -> dict[str, Any] | None:
    try:
...
def scan_root_incremental(
    ...
    for p in deterministic_quotes_files(root):
        files_seen += 1
        try:
            rel = str(p.relative_to(root))
        except Exception:
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build quotes_inventory_files for Agent02 v2")
    ...
    outdir = Path(args.outdir) if args.outdir else Path(
        rf"C:\TSIS_Data\02_backtest_SmallCaps\runs\backtest\quotes_v2_inventory\{utc_stamp()}_quotes_v2_inventory"
    )
    ...
    by_ticker = (
        ...
        .reset_index()
        if not all_df.empty else pd.DataFrame(columns=["ticker", "files_total", "task_keys", "roots_present", "date_min", "date_max", "bytes_total"])
    )

    files_parquet = outdir / "quotes_inventory_files.parquet"
    files_csv = outdir / "quotes_inventory_files.csv"
    by_ticker_parquet = outdir / "quotes_inventory_by_ticker.parquet"
    by_ticker_csv = outdir / "quotes_inventory_by_ticker.csv"
    summary_json = outdir / "quotes_inventory_summary.json"
    ...
    summary = {
        ...
        "outputs": {
            "quotes_inventory_files_parquet": str(files_parquet),
            "quotes_inventory_files_csv": str(files_csv),
            "quotes_inventory_by_ticker_parquet": str(by_ticker_parquet),
            "quotes_inventory_by_ticker_csv": str(by_ticker_csv),
