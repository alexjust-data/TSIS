...
def load_events(events_dir: Path, manifest: dict[str, Any], checkpoint: dict[str, Any]) -> tuple[pd.DataFrame, list[str], list[str]]:
    ...
    for entry in batch_entries:
        ...
        if p.exists() and p.parent == events_dir:
            confirmed_files.append(p)
            confirmed_names.append(p.name)

    if not confirmed_files:
        confirmed_files = sorted(events_dir.glob("batch_*.parquet"))
        confirmed_names = [p.name for p in confirmed_files]

    seen_set = set(confirmed_names)
    all_files = sorted(events_dir.glob("batch_*.parquet"))
    skipped_names = [p.name for p in all_files if p.name not in seen_set]

    if not confirmed_files:
        return pd.DataFrame(), [], skipped_names

    frames = [pd.read_parquet(p) for p in confirmed_files]
...
def derive_current(events_df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = ["file", "processed_at_utc", "batch_id"]
...
def detect_completion_status(manifest: dict[str, Any], checkpoint: dict[str, Any], validation_summary: dict[str, Any]) -> tuple[str, str]:
    ...
    checkpoint_batches = int(checkpoint.get("batches_written", 0) or 0)
...
def make_live_status(
    ...
    return {
        ...
        "current_materialized_from": str(outdir / "quotes_current.parquet"),
...
def write_outputs(outdir: Path, current_df: pd.DataFrame, retry_current_df: pd.DataFrame, retry_frozen_df: pd.DataFrame, live_status: dict[str, Any]) -> None:
    ...
    current_write.to_parquet(outdir / "quotes_current.parquet", index=False)
    current_write.to_csv(outdir / "quotes_current.csv", index=False)
    ...
    current_write.to_csv(outdir / "quotes_agent_strict_events_current.csv", index=False)
    retry_current_write.to_csv(outdir / "retry_queue_quotes_strict_current.csv", index=False)
    retry_frozen_write.to_csv(outdir / "retry_frozen_quotes_strict.csv", index=False)

    (outdir / "live_status_quotes_strict.json").write_text(json.dumps(live_status, indent=2), encoding="utf-8")


def main() -> None:
    ap = argparse.ArgumentParser(description="Materialize current/retry/live state from quotes v2 append-only events")
    ap.add_argument("--validation-outdir", required=True)
    ap.add_argument("--inventory-parquet", required=True)
    ap.add_argument("--outdir", default="")
    ap.add_argument("--run-id", default="quotes_v2_materialize")
    args = ap.parse_args()
