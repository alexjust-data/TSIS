...
def atomic_write_csv(path: Path, df: pd.DataFrame) -> None:
    ...
    for _ in range(40):
        try:
            ...
            last_exc = exc
            sleep(0.25)
    raise last_exc if last_exc is not None else RuntimeError(f'atomic_write_csv failed: {path}')
...
def verify_current_terminal_statuses(df: pd.DataFrame) -> tuple[pd.DataFrame, set[str], int]:
    if df.empty or 'task_key' not in df.columns or 'status' not in df.columns:
        return pd.DataFrame(), set(), 0

    cur = df.copy()
    cur['task_key'] = cur['task_key'].astype(str)
    ...
    for row in cur.itertuples(index=False):
        ...
        if status == 'DOWNLOADED_OK':
            file_path = Path(str(getattr(row, 'file', '') or ''))
            try:
                file_ok = file_path.exists() and file_path.stat().st_size > 0
            except Exception:
                file_ok = False
            if file_ok:
                verified_rows.append(row._asdict())
            else:
                repaired += 1
        elif status == 'DOWNLOADED_EMPTY':
            verified_rows.append(row._asdict())
        elif status in OK_STATUSES:
            verified_rows.append(row._asdict())

    verified_df = pd.DataFrame(verified_rows)
    already_ok = set(verified_df['task_key'].astype(str)) if (not verified_df.empty and 'task_key' in verified_df.columns) else set()
    return verified_df, already_ok, repaired
...
def persist_events(current_path: Path, history_path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    if not cur.empty and 'task_key' in cur.columns:
        ...
        merged = add
    atomic_write_csv(current_path, merged)

    write_header = not history_path.exists()
    add.to_csv(history_path, mode='a', header=write_header, index=False)
...
def main() -> None:
    ...
    if cfg.resume:
        verified_current_df, already_ok, repaired_terminal_rows = verify_current_terminal_statuses(current_df)
        if not verified_current_df.empty or len(current_df):
            atomic_write_csv(current_events, verified_current_df)
        current_df = verified_current_df
    else:
        already_ok = set()
    ...
    print(f'repaired_terminal_rows={repaired_terminal_rows}')
...
if __name__ == '__main__':
    try:
        ...
        for i, arg in enumerate(__import__('sys').argv):
            if arg == '--run-dir' and i + 1 < len(__import__('sys').argv):
                run_dir_arg = Path(__import__('sys').argv[i + 1])
                break
        if run_dir_arg is not None:
            run_dir_arg.mkdir(parents=True, exist_ok=True)
            (run_dir_arg / 'fatal_agent01_trades_ticks.log').write_text(traceback.format_exc(), encoding='utf-8')
        raise
