...
UNIVERSE_FULL = Path(
    globals().get(
        "UNIVERSE_FULL",
        r"C:\TSIS_Data\02_backtest_SmallCaps\data\reference\universe_pti\tickers_2005_2026.parquet",
    )
)
UNIVERSE_UPPER = Path(
    globals().get(
...
EXCLUDED_PARQUET = Path(
    globals().get(
        "EXCLUDED_PARQUET",
        r"C:\TSIS_Data\02_backtest_SmallCaps\data\reference\universe_pti\tickers_2005_2026_upper_excluded_overview404.parquet",
    )
)
OUTDIR = Path(
    globals().get(
        "OUTDIR",
        r"C:\TSIS_Data\02_backtest_SmallCaps\data\reference\universe_pti",
    )
)
