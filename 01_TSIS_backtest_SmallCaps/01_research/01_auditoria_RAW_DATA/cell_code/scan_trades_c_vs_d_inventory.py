...
def scan_root(root: Path, label: str, progress_every: int = 100000) -> pd.DataFrame:
    ...
    for p in root.rglob("market.parquet"):
        count += 1
        try:
...
def main() -> None:
    ...
    outdir = Path(args.outdir) if args.outdir else Path(
        rf"C:\TSIS_Data\02_backtest_SmallCaps\runs\backtest\trades_c_vs_d_inventory\{utc_stamp()}_scan_trades_c_vs_d_inventory"
    )
    ...
    c_inv_csv = outdir / "trades_c_inventory.csv"
    d_inv_csv = outdir / "trades_d_inventory.csv"
    compare_csv = outdir / "trades_c_vs_d_comparison.csv"
    compare_parquet = outdir / "trades_c_vs_d_comparison.parquet"
    overlap_csv = outdir / "trades_overlap.csv"
    only_c_csv = outdir / "trades_only_in_c.csv"
    only_d_csv = outdir / "trades_only_in_d.csv"
    summary_json = outdir / "trades_c_vs_d_summary.json"
    ...
    compare["in_c"] = compare["in_c"].fillna(False).astype(bool)
    compare["in_d"] = compare["in_d"].fillna(False).astype(bool)
