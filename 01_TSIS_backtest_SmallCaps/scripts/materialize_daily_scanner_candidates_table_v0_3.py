...
CONTRACTS = {
    ...
    "target_contract": (
        ...
        "daily_scanner_candidates_table_target_contract_v0_3.md"
    ),
    "dataset_contract": (
    ...
    "scanner_framework": (
        "01_foundations/module_contracts/outputs/"
        "scanner_framework_and_definitions_contract_v0_3.md"
    ),
}

DATASET_ID = "daily_scanner_candidates_table_v0_3"
DATASET_DIR_NAME = "daily_scanner_candidates_table_v0_3_candidate_replay"
SCHEMA_VERSION = "daily_scanner_candidates_table_v0_3_in_play_momentum_extension"
QUALITY_POLICY_VERSION = "daily_scanner_candidates_table_policy_v0_3"
SCANNER_POLICY_VERSION = "scanner_framework_v0_3"
...
def _run_id() -> str:
    return "daily_scanner_candidates_replay_v0_3_" + datetime.now(timezone.utc).strftime(
        "%Y%m%dT%H%M%SZ"
    )
...
def _scanner_config_paths(scanner_definitions_dir: Path) -> list[Path]:
    paths = [
        ...
        scanner_definitions_dir / "in_play_momentum_candidate_denominator_v0_3.yaml",
        scanner_definitions_dir / "trade_station_like_profile_v0_3.yaml",
    ]
...
def _build_sql(args: argparse.Namespace, configs: dict[str, dict[str, Any]]) -> str:
    ...
    base_cfg = configs["base_eligible_smallcap_denominator_v0_3"]
    in_play_cfg = configs["in_play_momentum_candidate_denominator_v0_3"]
    ts_profile = configs["trade_station_like_profile_v0_3"]
    ...
    push_threshold = float(in_play_cfg["movement_policy"]["minimum_push_move_pct"])
    volume_min = float(in_play_cfg["tradability_policy"]["minimum_volume_today"])
    dollar_min = float(in_play_cfg["tradability_policy"]["minimum_dollar_volume_today"])
    display_top_n = int(in_play_cfg["selection"]["top_n_for_display"])
...
base as (
    select
        *,
        ...
        ((high_price / previous_close) - 1.0) * 100.0 as daily_high_vs_prev_close_pct,
        ((low_price / previous_close) - 1.0) * 100.0 as daily_low_vs_prev_close_pct,
        greatest(
            coalesce(((high_price / previous_close) - 1.0) * 100.0, -999999.0),
            coalesce(pct_chg_1d, -999999.0),
            coalesce(gap_pct, -999999.0)
        ) as in_play_motion_pct,
        greatest(
            coalesce(((high_price / previous_close) - 1.0) * 100.0, 0.0),
            coalesce(pct_chg_1d, 0.0),
            coalesce(gap_pct, 0.0)
        )
            + case
                when volume_today >= {volume_min} then 10.0
                when dollar_volume_today >= {dollar_min} then 5.0
                else 0.0
            end
            + case
                when dollar_volume_to_time > 0 then ln(dollar_volume_to_time + 1.0) / ln(10.0)
                else 0.0
            end as in_play_momentum_score
    from base_raw
),
reasoned as (
    select
        *,
        ...
        daily_high_vs_prev_close_pct >= {push_threshold} as reason_daily_high_push_50,
        pct_chg_1d >= {push_threshold} as reason_pct_chg_1d_move,
        gap_pct >= {push_threshold} as reason_gap_pct_move,
        (
            daily_high_vs_prev_close_pct >= {push_threshold}
            or pct_chg_1d >= {push_threshold}
            or gap_pct >= {push_threshold}
        ) as in_play_motion_threshold_passed,
        (
            volume_today >= {volume_min}
            or dollar_volume_today >= {dollar_min}
        ) as in_play_volume_tradability_passed,
        false as reason_volume_acceleration,
        false as reason_rvol_to_time,
        false as reason_premarket_push_50,
        false as reason_regular_push_50,
        false as reason_afterhours_push_50,
        false as reason_extended_session_push_50,
...
rank_composite as (
    select
        ...
        row_number() over (
            partition by session_date
            order by in_play_momentum_score desc nulls last, ticker asc
    ...
    select
        *,
        ...
        base_all_filters_passed
            ...
            and rank_pct_chg_1d <= {display_top_n}
        ...
        base_all_filters_passed
            and in_play_motion_threshold_passed
            and in_play_volume_tradability_passed
            as selected_in_play_momentum_candidate,
        false as selected_das_research_profile
    from joined
),
with_reasons as (
    select
        *,
        nullif(concat_ws('|',
            case when reason_daily_high_push_50 then 'reason_daily_high_push_50' end,
            ...
            case when in_play_volume_tradability_passed then 'reason_in_play_volume_tradability_passed' end,
            ...
            case when reason_premarket_push_50 then 'reason_premarket_push_50' end,
            case when reason_regular_push_50 then 'reason_regular_push_50' end,
            case when reason_afterhours_push_50 then 'reason_afterhours_push_50' end,
            case when reason_extended_session_push_50 then 'reason_extended_session_push_50' end,
            ...
            case when selected_in_play_momentum_candidate then 'profile_in_play_momentum_candidate' end
        ), '') as candidate_reasons,
        (
            case when reason_daily_high_push_50 then 1 else 0 end
            + case when reason_pct_chg_1d_move then 1 else 0 end
            + case when reason_gap_pct_move then 1 else 0 end
            + case when in_play_volume_tradability_passed then 1 else 0 end
            + case when reason_volume_acceleration then 1 else 0 end
            + case when reason_rvol_to_time then 1 else 0 end
            + case when reason_premarket_push_50 then 1 else 0 end
            + case when reason_regular_push_50 then 1 else 0 end
            + case when reason_afterhours_push_50 then 1 else 0 end
            + case when reason_extended_session_push_50 then 1 else 0 end
            ...
            + case when selected_in_play_momentum_candidate then 1 else 0 end
        ) as candidate_reason_count,
        selected_in_play_momentum_candidate as selected_any_profile
    from profiled
),
with_counts as (
    select
...
select
    ...
    'v0_3' as scanner_definition_version,
    'daily_in_play' as scanner_family,
    'smallcap_in_play_momentum_scanner' as scanner_name,
    'historical_replay' as scanner_mode,
    'base_eligible_plus_in_play_momentum_denominator' as scanner_role,
    instrument_id,
    ticker,
    session_date,
    as_of_utc,
    ...
    is_common_stock,
    is_lt1b_operational,
    'smallcap_common_stock_under_100m_in_play_momentum_v0_3' as universe_definition_id,
    'base_eligible_smallcap_denominator_v0_3' as base_universe_definition_id,
    'v0_3' as base_universe_definition_version,
    'scanner_profile_set_v0_3' as scanner_profile_set_id,
    ...
    'base_then_in_play_momentum_denominator' as scanner_profile_semantics,
    ...
    {push_threshold}::double as percent_change_min_threshold_pct,
    'tradability_not_alpha' as dollar_volume_profile_semantic_role,
    'removed_from_global_scanner_strategy_overlay_only' as das_research_profile_status,
    'controlled_daily_replay' as population_scope,
    population_denominator_count,
    evaluated_candidate_count,
    selected_candidate_count,
    false as full_universe_claim,
    {display_top_n}::integer as top_n,
    rank_composite_in_play as rank,
    'in_play_momentum_score' as rank_metric,
    in_play_momentum_score as rank_metric_value,
    rank_composite_in_play <= {display_top_n} as included_in_top_n,
    rank_pct_chg_1d,
    ...
    selected_in_play_momentum_candidate as selected_broad_discovery,
    ...
    selected_in_play_momentum_candidate as selected_by_composite_in_play_rank,
    ...
    {push_threshold}::double as in_play_momentum_min_push_pct,
    {volume_min}::double as in_play_volume_min_shares,
    {dollar_min}::double as in_play_dollar_volume_min_usd,
    'daily_eod_proxy' as in_play_detection_scope,
    'not_available_without_intraday_segment_builder' as in_play_segment_detection_state,
    cast(null as varchar) as in_play_first_push_segment,
    cast(null as varchar) as in_play_first_cross_50_ts,
    cast(null as varchar) as in_play_max_move_segment,
    volume_today,
    dollar_volume_today,
    volume_to_time,
    volume_since_04_00,
    ...
    {volume_min}::double as volume_min_filter,
    {market_cap_max}::double as market_cap_max_filter,
    price_filter_passed,
    in_play_volume_tradability_passed as volume_filter_passed,
    trade_station_volume_filter_passed,
    ...
    case
        ...
        when selected_in_play_momentum_candidate and rank_composite_in_play <= {display_top_n}
            then 'selected_in_play_momentum_top_n'
        when selected_in_play_momentum_candidate then 'selected_in_play_momentum_threshold'
        else 'base_eligible_not_in_play'
    ...
    selected_in_play_momentum_candidate and not (rank_composite_in_play <= {display_top_n})
...
def _write_summary(con: duckdb.DuckDBPyConnection, path: Path) -> list[dict[str, Any]]:
    rows = con.sql(
        ...
        select
            ...
            sum(case when selected_in_play_momentum_candidate then 1 else 0 end)::bigint
                as in_play_momentum_candidate_rows,
    ...
    columns = [
        ...
        "instruments",
            "selected_rows",
            "in_play_momentum_candidate_rows",
...
def _validations(con: duckdb.DuckDBPyConnection) -> dict[str, Any]:
    row = con.sql(
        ...
        select
            ...
            sum(case when scanner_selection_state = 'base_eligible_not_in_play' then 1 else 0 end)::bigint
                as base_eligible_not_in_play_rows,
            sum(case when selected_any_profile then 1 else 0 end)::bigint as selected_any_profile_rows,
            sum(case when selected_in_play_momentum_candidate then 1 else 0 end)::bigint
                as selected_in_play_momentum_candidate_rows,
            sum(case when selected_trade_station_like_profile then 1 else 0 end)::bigint
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    summary_path = args.output_root / "_daily_scanner_candidates_table_summary_v0_3_candidate_replay.csv"
    manifest_path = args.output_root / "_daily_scanner_candidates_table_manifest_v0_3_candidate_replay.json"
    ...
    manifest = {
        ...
        "contract_alignment": {
            ...
            "in_play_denominator_semantics": "base_plus_strong_motion_plus_tradability",
            "in_play_detection_scope": "daily_eod_proxy",
            "in_play_segment_detection_state": "not_available_without_intraday_segment_builder",
            ...
            "das_research_profile_status": "removed_from_global_scanner_strategy_overlay_only",
        ...
        "limitations": [
            "This is a controlled daily EOD proxy replay, not an official intraday/segment scanner.",
            "The controlled replay detects strong daily high/close/gap movement; it cannot certify whether the first frontside occurred in premarket, regular or afterhours.",
            "Afterhours, premarket, segment, news and halt candidate reasons are false until scoped sources are joined.",
            ...
            "In-play momentum selection requires the configured 50 percent movement threshold plus minimum volume/tradability.",
            "Dollar-volume selection is a tradability/economic-activity profile, not an alpha or setup-quality signal.",
            "DAS is removed from the global scanner and must live as a strategy overlay.",
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Materialize controlled daily scanner candidate replay v0.3."
    )
...
def main(argv: list[str] | None = None) -> None:
    ...
    print("Daily scanner candidates replay v0.3 completed.")
    ...
    print(f"Selected in-play momentum rows: {validations['selected_in_play_momentum_candidate_rows']}")
    print(f"Selected TradeStation-like profile rows: {validations['selected_trade_station_like_profile_rows']}")
    print(f"Selected DAS research profile rows: {validations['selected_das_research_profile_rows']} (compatibility column; expected 0)")
