"""Bounded physical runner for the BT-GATE-015 V0.4 single-use probe."""
...
from .physical_consumer_v0_4 import PhysicalEventStateConsumerV04
...
class PhysicalRunnerV04:
    ...
    def validate_preconsumption(
        ...
        correction = specification["semantic_correction"]
        dependency = specification["market_state_dependency"]
        if (
            correction.get("status")
            != "IMPLEMENTED_IN_V0_4_PENDING_EXTERNAL_PREEXECUTION_REVIEW"
            or correction.get("physical_provenance_binding_field")
            != "market_state_dependency_dataset_fingerprint"
            or correction.get("availability_evidence_binding_field")
            != "market_state_availability_evidence_dataset_fingerprint"
            or correction.get("distinct_domains_required") is not True
            or dependency["dependency_dataset_fingerprint"]
            == dependency["availability_evidence_dataset_fingerprint"]
            or event_records[0]["market_state_dependency_dataset_fingerprint"]
            ...
            raise EventStateContractError(
                "FAIL_BT_GATE_015_CLOSED_CONFIGURATION_MISMATCH",
                "dataset fingerprint domain binding",
            )
    ...
    def execute(
        ...
        try:
            ...
            AuthorizationV04.write_progress(run_directory, progress)
            ...
            physical_consumer = PhysicalEventStateConsumerV04(
                specification["selection"]["event_state_record_id"],
                specification["selection"]["event_state_record_fingerprint"],
            )
            ...
            AuthorizationV04.record_success(run_directory, progress)
            return final
            ...
            AuthorizationV04.record_failure(
