...
from tsis_backtest.market_state.physical_authorization_v0_5 import (
    AUTHORIZATION_ID,
    CONSUMED_STATUS,
    CONSUMER_ID,
...
from tsis_backtest.market_state.physical_runner_v0_5 import (
    CONFIG_FIELDS,
...
class PhysicalV05Tests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.barrier_path = (
            self.engine_root
            / "configs/fixtures/"
            "BT_GATE_014_PHYSICAL_INTEGRATION_BARRIERS_V0_5.json"
        )
        write_json(
            self.barrier_path,
            {
                ...
                "fixture_id": (
                    "BT_GATE_014_PHYSICAL_INTEGRATION_BARRIERS_V0_5"
                ),
    ...
    def _configuration(self) -> dict:
        configuration = {
            ...
            "restriction_domain_binding_id": "market_state_restriction_domain_binding_clarification_v0_1",
            "restriction_domain_binding_sha256": "b3ebace6fab7cb0a690be165f0a1706a5906dc7a729488fa1139af00c02e53a9",
    ...
    def execute(
        ...
        *,
        reader: Mock | None = None,
        authorization: AuthorizationV05 | None = None,
        configuration: dict | None = None,
    ) -> dict:
        return self.runner(rows, reader=reader).execute(
            self.tsis_root,
            configuration or self.configuration,
            authorization or AuthorizationV05(self.state_path),
            self.bindings,
        )
    ...
    def test_receipt_write_failure_is_durably_evidenced(self) -> None:
        ...
        authorization = AuthorizationV05(self.state_path)
        with patch.object(
            authorization,
            "_write_receipt",
    ...
    def test_state_transition_failure_keeps_armed_guard(self) -> None:
        import tsis_backtest.market_state.physical_authorization_v0_5 as auth_module
    ...
    def test_failure_after_physical_rows_loaded_reports_real_progress(self) -> None:
        ...
        self.assertEqual(diagnostics["replay_consumption_restrictions"], list(EXPECTED_RESTRICTIONS))
...
class PhysicalV05CanonicalBindingTests(unittest.TestCase):
    def setUp(self) -> None:
        self.state_path = REPOSITORY_ROOT / "configs/authorizations/bt_gate_014_single_use_physical_consumer_authorization_v0_5.json"
        self.state = json.loads(self.state_path.read_text(encoding="utf-8"))

    def test_v05_identity_is_consumed_once(self) -> None:
        configuration = json.loads((REPOSITORY_ROOT / "configs/runs/bt_gate_014_single_use_physical_market_state_consumer_v0_5.json").read_text(encoding="utf-8"))
        self.assertEqual(AUTHORIZATION_ID, self.state["authorization_id"])
        self.assertEqual(AUTHORIZATION_ID, configuration["authorization_id"])
        self.assertEqual(self.state["authorization_id"], "BT-GATE-014-SINGLE-USE-PHYSICAL-AUTHORIZATION-V0-5")
        self.assertEqual(
            self.state["status"],
            "CONSUMED_BY_RUN_bt_gate_014_single_use_physical_market_state_consumer_v0_5",
        )
        self.assertEqual(
            self.state["consumed_by_run_id"],
            "bt_gate_014_single_use_physical_market_state_consumer_v0_5",
        )

    def test_v05_closed_binding_hashes_resolve(self) -> None:
        for relative, expected in self.state["binding_sha256"].items():
            path = REPOSITORY_ROOT / relative
            self.assertTrue(path.is_file(), relative)
            self.assertEqual(sha256(path), expected, relative)
        document = REPOSITORY_ROOT / "docs/00_system/18_BT_GATE_014_SINGLE_USE_PHYSICAL_CONSUMER_AUTHORIZATION_V0_5.md"
        configuration = REPOSITORY_ROOT / "configs/runs/bt_gate_014_single_use_physical_market_state_consumer_v0_5.json"
        self.assertEqual(sha256(document), self.state["authorization_document_sha256"])
        self.assertEqual(sha256(configuration), self.state["configuration_sha256"])

    def test_production_spec_validates_canonical_v05_preconsumption(self) -> None:
        configuration = json.loads(
            (
                REPOSITORY_ROOT
                / "configs/runs/bt_gate_014_single_use_physical_market_state_consumer_v0_5.json"
            ).read_text(encoding="utf-8")
        )
        PhysicalRunnerV05(PRODUCTION_SPEC).validate_preconsumption(
            REPOSITORY_ROOT.parent,
            configuration,
            self.state["binding_sha256"],
        )
        self.assertEqual(
            configuration["restriction_domain_binding_id"],
            "market_state_restriction_domain_binding_clarification_v0_1",
        )
        self.assertEqual(
            configuration["restriction_domain_binding_sha256"],
            "b3ebace6fab7cb0a690be165f0a1706a5906dc7a729488fa1139af00c02e53a9",
        )
        self.assertEqual(len(configuration["physical_input_sha256"]), 10)

    def test_v03_and_v05_consumed_evidence_exists(self) -> None:
        ...
        run05 = REPOSITORY_ROOT / "runs/bt_gate_014_single_use_physical_market_state_consumer_v0_5"
        for name in (
            "authorization_consumption_receipt.json",
            ...
            self.assertTrue((run05 / name).is_file(), name)
        final = json.loads((run05 / "final_manifest.json").read_text(encoding="utf-8"))
        self.assertEqual(final["validation_status"], "PASS")
        self.assertEqual(final["physical_state_rows_read"], 2)
