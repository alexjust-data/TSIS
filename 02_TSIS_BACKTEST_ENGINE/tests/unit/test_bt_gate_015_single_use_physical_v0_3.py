...
from tsis_backtest.event_state.physical_authorization_v0_3 import (
    AUTHORIZATION_ID,
    CONSUMED_STATUS,
    RUN_ID,
    AuthorizationV03,
)
from tsis_backtest.event_state.physical_consumer_v0_3 import (
    PhysicalEventStateConsumerV03,
    physical_record_fingerprint,
)
from tsis_backtest.event_state.physical_runner_v0_3 import PhysicalRunnerV03
...
class PhysicalV03Fixture:
    def __init__(self) -> None:
        ...
        self.barrier_path = (
            self.engine_root
            / "configs/fixtures/BT_GATE_015_PHYSICAL_INTEGRATION_BARRIERS_V0_3.json"
        )
    ...
    def _build_inputs(self) -> None:
        ...
        row["source_market_state_candidate_dataset_fingerprint"] = market_record[
        ...
        self.market_document = {
            "sidecar_id": "synthetic_market_state_dependency_document_v0_3",
    ...
    def execute(self, reader=None):
        return PhysicalRunnerV03(reader).execute(
            self.tsis_root,
            self.config_path,
            AuthorizationV03(self.state_path, self.spec_path),
        )


class PhysicalEventStateV03Tests(unittest.TestCase):
    def setUp(self) -> None:
        self.fixture = PhysicalV03Fixture()

    def tearDown(self) -> None:
        self.fixture.close()

    def test_real_v03_classes_complete_synthetic_eight_to_one_path(self) -> None:
    ...
    def test_preconsumption_never_calls_candidate_reader(self) -> None:
        ...
        runner = PhysicalRunnerV03(reader)
        runner.validate_preconsumption(
            self.fixture.tsis_root,
            self.fixture.config_path,
            AuthorizationV03(self.fixture.state_path, self.fixture.spec_path),
        )
    ...
    def test_receipt_write_failure_is_durable_and_never_reads(self) -> None:
        ...
        with patch.object(
            ...
            with self.assertRaisesRegex(OSError, "receipt write failure"):
                PhysicalRunnerV03(reader).execute(
...
class PhysicalEventStateV03CanonicalTests(unittest.TestCase):
    ...
    def test_canonical_state_records_consumed_failure(self) -> None:
        state = json.loads(CANONICAL_STATE.read_text())
        self.assertEqual(state["status"], f"CONSUMED_BY_RUN_{RUN_ID}")
        ...
        self.assertEqual(failure["failure_code"], "FAIL_EVENT_STATE_IDENTITY_MISMATCH")
        self.assertEqual(failure["physical_state_records_scanned"], 8)
        self.assertEqual(failure["physical_state_rows_selected"], 1)
        self.assertEqual(failure["event_state_events_emitted"], 0)
