...
def build_final_df(tax_df):
    ...
    final_df["dup_excess_ratio_pct_num"] = pd.to_numeric(final_df.get("m.duplicate_excess_ratio_pct"), errors="coerce").fillna(0)
    final_df["off_session_trade_pct_num"] = pd.to_numeric(final_df.get("m.off_session_trade_pct"), errors="coerce").fillna(0)
...
def export_final_outputs(final_df, final_summary, export_dir):
    ...
    final_df.to_parquet(export_dir / "trades_full_root_cause_final_bucket.parquet", index=False)
    final_df.to_csv(export_dir / "trades_full_root_cause_final_bucket.csv", index=False)
    final_summary.to_parquet(export_dir / "trades_full_root_cause_final_bucket_summary.parquet", index=False)
    final_summary.to_csv(export_dir / "trades_full_root_cause_final_bucket_summary.csv", index=False)
    return export_dir


def run_full_final_bucket(tax_df, export_dir, write_exports: bool = True):
