...
def build_exec_summary_from_counts(
    selected_files: int,
    sev_counts_full: pd.DataFrame,
    hard_issue_counts_full: pd.DataFrame,
    issue_evidence_full: pd.DataFrame,
):
    sev_map = sev_counts_full.set_index("severity")["files"].to_dict() if not sev_counts_full.empty else {}
    hard_total_full = int(sev_map.get("HARD_FAIL", 0))
    pass_total_full = int(sev_map.get("PASS", 0))
    soft_total_full = int(sev_map.get("SOFT_FAIL", 0))
    top_issue_files_full = int(hard_issue_counts_full.iloc[0]["files"]) if not hard_issue_counts_full.empty else 0
    top_issue_name_full = hard_issue_counts_full.iloc[0]["issue"] if not hard_issue_counts_full.empty else None
    top_issue_row_full = issue_evidence_full.iloc[0].to_dict() if not issue_evidence_full.empty else {}
    ...
    exec_summary_full = pd.DataFrame([
        {
            ...
            "pass_rate_pct": round(100.0 * pass_total_full / max(selected_files, 1), 3),
            "soft_fail_rate_pct": round(100.0 * soft_total_full / max(selected_files, 1), 3),
            "hard_fail_rate_pct": round(100.0 * hard_total_full / max(selected_files, 1), 3),
            "top_hard_issue": top_issue_name_full,
            "top_hard_issue_files": top_issue_files_full,
            "top_hard_issue_pct_of_hard": round(100.0 * top_issue_files_full / max(hard_total_full, 1), 3),
            "top_hard_issue_tickers": int(top_issue_row_full.get("tickers", 0)),
