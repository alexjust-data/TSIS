# Canonical full <1B> closeout viewer for the final 57f cache.
# 58c is retained only for backward compatibility with earlier notebook wiring.
...
def plot_layer2_full(summary: pd.DataFrame) -> None:
    ...
    with plt.rc_context(PLOT_RC):
        fig, axes = plt.subplots(1, 2, figsize=(11.2, 4.2), gridspec_kw={"width_ratios": [1.2, 1.0]})
        ax0, ax1 = axes

        bars0 = ax0.barh(["files_total_full_raw"], [total], color="#4C72B0", height=0.42)
        ax0.set_title("Capa 2 full | Universo procesado")
        ax0.set_xlabel("files")
        ax0.grid(axis="x", alpha=0.2)
        for bar in bars0:
            value = bar.get_width()
            ax0.text(value, bar.get_y() + bar.get_height() / 2, f" {int(value):,}", va="center", ha="left", fontsize=8)

        pct_labels = ["files_with_1m_reference_pct", "files_with_daily_reference_pct"]
        pct_values = [one_m, daily]
        bars1 = ax1.barh(pct_labels, pct_values, color=["#55A868", "#C44E52"], height=0.42)
        ax1.set_title("Capa 2 full | Cobertura de referencias")
        ax1.set_xlabel("pct de files")
        ax1.set_xlim(0, 100)
        ax1.grid(axis="x", alpha=0.2)
        ax1.invert_yaxis()
        for bar, value in zip(bars1, pct_values):
            ax1.text(value, bar.get_y() + bar.get_height() / 2, f" {value:.2f}%", va="center", ha="left", fontsize=8)
