...
STATE_FILE = Path(
    globals().get(
        "STATE_FILE",
        r"C:\TSIS_Data\02_backtest_SmallCaps\runs\polygon_realtime_audit\quotes_agent_strict_state.json",
    )
)
EVENTS_CSV = Path(
    globals().get(
        "EVENTS_CSV",
        r"C:\TSIS_Data\02_backtest_SmallCaps\runs\polygon_realtime_audit\quotes_agent_strict_events.csv",
    )
)
GRANULAR_DIR = Path(
    globals().get(
        "GRANULAR_DIR",
        r"C:\TSIS_Data\02_backtest_SmallCaps\runs\polygon_realtime_audit\granular_strict",
    )
)
RETRY_POLICY = globals().get("RETRY_POLICY", "hard_only")  # hard_only | hard_and_soft
RETRY_QUEUE_CSV = Path(globals().get("RETRY_QUEUE_CSV", r"C:\TSIS_Data\02_backtest_SmallCaps\runs\polygon_realtime_audit\retry_queue_quotes_strict.csv"))
RETRY_QUEUE_PARQUET = Path(globals().get("RETRY_QUEUE_PARQUET", r"C:\TSIS_Data\02_backtest_SmallCaps\runs\polygon_realtime_audit\retry_queue_quotes_strict.parquet"))
