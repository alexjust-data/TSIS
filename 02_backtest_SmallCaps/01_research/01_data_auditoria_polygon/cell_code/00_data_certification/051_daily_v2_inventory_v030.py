...
PATTERN_DAILY = re.compile(
    r"^ticker=(?P<ticker>[^\\/]+)[\\/]+year=(?P<year>\d{4})[\\/]+day_aggs_(?P<filename_ticker>[^_]+)_(?P<filename_year>\d{4})\.parquet$",
    re.IGNORECASE,
)
INVENTORY_COLUMNS = [
    "root",
...
def deterministic_daily_files(root: Path):
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for name in filenames:
            if not name.lower().endswith(".parquet"):
                continue
            if not name.lower().startswith("day_aggs_"):
                continue
...
def build_row(root: Path, label: str, file_path: Path) -> dict[str, Any] | None:
    ...
    return {
        ...
        "task_key": f"{parsed['ticker']}|{parsed['year']}",
        "size_bytes": size,
        "mtime_utc": mtime_utc,
...
def persist_batch(
    ...
    if not df.empty:
        df = df.sort_values(["ticker", "year", "file"]).reset_index(drop=True)
...
def scan_root_incremental(
    ...
    for file_path in deterministic_daily_files(root):
        files_seen += 1
        try:
            rel = str(file_path.relative_to(root))
        except Exception:
...
def load_batch_frames(outdir: Path) -> pd.DataFrame:
    ...
    if not df.empty:
        df = df.sort_values(["ticker", "year", "root", "file"]).reset_index(drop=True)
    return df
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build daily_inventory_files for Agent02 daily v2")
    ap.add_argument("--c-root", default=r"C:\TSIS_Data\data\ohlcv_daily")
    ap.add_argument("--d-root", default=r"D:\ohlcv_daily")
    ...
    if c_root.exists():
        roots.append(("C", c_root))
    if d_root.exists():
        roots.append(("D", d_root))
    if not roots:
        raise SystemExit("No valid roots found for daily inventory")

    outdir = Path(args.outdir) if args.outdir else Path(
        rf"C:\TSIS_Data\02_backtest_SmallCaps\runs\backtest\daily_v2_inventory\{utc_stamp()}_daily_v2_inventory"
    )
    ...
    by_ticker = (
        ...
        if not all_df.empty
            ...
            columns=["ticker", "files_total", "task_keys", "roots_present", "year_min", "year_max", "bytes_total"]
        )
    )

    files_parquet = outdir / "daily_inventory_files.parquet"
    files_csv = outdir / "daily_inventory_files.csv"
    by_ticker_parquet = outdir / "daily_inventory_by_ticker.parquet"
    by_ticker_csv = outdir / "daily_inventory_by_ticker.csv"
    summary_json = outdir / "daily_inventory_summary.json"
    ...
    summary = {
        ...
        "outputs": {
            "daily_inventory_files_parquet": str(files_parquet),
            "daily_inventory_files_csv": str(files_csv),
            "daily_inventory_by_ticker_parquet": str(by_ticker_parquet),
            "daily_inventory_by_ticker_csv": str(by_ticker_csv),
