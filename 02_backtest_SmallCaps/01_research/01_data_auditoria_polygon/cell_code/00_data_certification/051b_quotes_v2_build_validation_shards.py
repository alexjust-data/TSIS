...
def main() -> None:
    ap = argparse.ArgumentParser(description="Build deterministic quotes validation shards from inventory parquet")
    ...
    for shard_id in range(int(args.num_shards)):
        ...
        shard_path = outdir / f"quotes_inventory.shard_{shard_id + 1:02d}_of_{int(args.num_shards):02d}.parquet"
    ...
    write_json(outdir / "quotes_validation_shards_manifest.json", shard_manifest)

    summary_df = pd.DataFrame(shard_rows)
    summary_df.to_csv(outdir / "quotes_validation_shards_summary.csv", index=False)
