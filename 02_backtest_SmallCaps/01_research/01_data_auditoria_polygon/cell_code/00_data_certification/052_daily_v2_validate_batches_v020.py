...
VALIDATOR_PATH = SCRIPT_DIR / "050_daily_v2_validate_file_v020.py"
...
def load_validator_module():
    spec = importlib.util.spec_from_file_location("daily_v2_validate_file_mod", VALIDATOR_PATH)
...
VALIDATOR_VERSION = getattr(VALIDATOR_MOD, "VALIDATOR_VERSION", "daily_v2_validate_file/unknown")
validate_daily_file = getattr(VALIDATOR_MOD, "validate_daily_file")
...
def worker_validate(task: dict[str, Any]) -> dict[str, Any]:
    return validate_daily_file(
        ...
        min_coverage_ratio_warn=float(task["min_coverage_ratio_warn"]),
...
def build_outdir(base_outdir: str) -> Path:
    if str(base_outdir).strip():
        ...
        outdir = Path(r"C:\TSIS_Data\02_backtest_SmallCaps\runs\backtest\daily_v2_validation") / f"{stamp}_daily_v2_validation"
...
def write_manifest_exports(outdir: Path, manifest: dict[str, Any]) -> None:
    ...
    manifest_df.to_csv(outdir / "batch_manifest_daily_v2.csv", index=False)
    if not manifest_df.empty:
        manifest_df.to_parquet(outdir / "batch_manifest_daily_v2.parquet", index=False)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Validate daily inventory in parallel and write append-only batch events")
    ap.add_argument("--inventory-parquet", required=True)
    ap.add_argument("--outdir", default="")
    ap.add_argument("--run-id", default="manual_daily_v2")
    ...
    for idx, batch_df in enumerate(batches, start=1):
        ...
        for row in batch_df.to_dict(orient="records"):
            ...
            row["min_coverage_ratio_warn"] = args.min_coverage_ratio_warn
