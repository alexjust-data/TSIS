...
VALIDATOR_PATH = SCRIPT_DIR / "050_quotes_v2_validate_file.py"
...
def load_validator_module():
    spec = importlib.util.spec_from_file_location("quotes_v2_validate_file_mod", VALIDATOR_PATH)
...
VALIDATOR_VERSION = getattr(VALIDATOR_MOD, "VALIDATOR_VERSION", "quotes_v2_validate_file/unknown")
validate_quotes_file = getattr(VALIDATOR_MOD, "validate_quotes_file")
...
def worker_validate(task: dict[str, Any]) -> dict[str, Any]:
    return validate_quotes_file(
        ...
        crossed_ratio_threshold_pct=float(task["crossed_ratio_threshold_pct"]),
        hard_fail_crossed_pct=float(task["hard_fail_crossed_pct"]),
        hard_fail_ask_integer_pct=float(task["hard_fail_ask_integer_pct"]),
        hard_fail_ask_int_crossed_pct=float(task["hard_fail_ask_int_crossed_pct"]),
    )
...
def build_outdir(base_outdir: str) -> Path:
    if str(base_outdir).strip():
        ...
        outdir = Path(r"C:\TSIS_Data\02_backtest_SmallCaps\runs\backtest\quotes_v2_validation") / f"{stamp}_quotes_v2_validation"
...
def merge_manifest_batch_files(manifest: dict[str, Any], events_dir: Path) -> dict[str, Any]:
    ...
    for p in sorted(events_dir.glob("batch_*.parquet")):
        batch_id = p.stem
...
def write_manifest_exports(outdir: Path, manifest: dict[str, Any]) -> None:
    ...
    manifest_df.to_csv(outdir / "batch_manifest_quotes_v2.csv", index=False)
    if not manifest_df.empty:
        manifest_df.to_parquet(outdir / "batch_manifest_quotes_v2.parquet", index=False)
...
def recompute_total_counts(manifest: dict[str, Any], events_dir: Path) -> dict[str, int]:
    total: dict[str, int] = {}
    for entry in manifest.get("batch_files", []):
        p = Path(str(entry.get("path", "")).strip())
        if not p.exists():
            candidate = events_dir / f"{entry.get('batch_id')}.parquet"
            p = candidate
        counts = load_severity_counts_from_parquet(p)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Validate quotes inventory in parallel and write append-only batch events")
    ap.add_argument("--inventory-parquet", required=True)
    ap.add_argument("--outdir", default="")
    ap.add_argument("--run-id", default="manual_quotes_v2")
    ...
    for idx, batch_df in enumerate(batches, start=1):
        ...
        for row in batch_df.to_dict(orient="records"):
            ...
            row["crossed_ratio_threshold_pct"] = args.crossed_ratio_threshold_pct
            row["hard_fail_crossed_pct"] = args.hard_fail_crossed_pct
            row["hard_fail_ask_integer_pct"] = args.hard_fail_ask_integer_pct
            row["hard_fail_ask_int_crossed_pct"] = args.hard_fail_ask_int_crossed_pct
            tasks.append(row)
        ...
        for k, v in batch_summary["severity_counts"].items():
