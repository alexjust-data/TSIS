...
VALIDATOR_PATH = SCRIPT_DIR / "050_trades_v2_validate_file_v020.py"
...
def load_validator_module():
    spec = importlib.util.spec_from_file_location("trades_v2_validate_file_mod", VALIDATOR_PATH)
...
VALIDATOR_VERSION = getattr(VALIDATOR_MOD, "VALIDATOR_VERSION", "trades_v2_validate_file/unknown")
validate_trades_file = getattr(VALIDATOR_MOD, "validate_trades_file")
...
def choose_optional_root(root_path: str) -> Path | None:
    s = str(root_path or "").strip()
    return Path(s) if s else None


def worker_validate(task: dict[str, Any]) -> dict[str, Any]:
    return validate_trades_file(
        ...
        ohlcv_daily_root=choose_optional_root(task.get("ohlcv_daily_root", "")),
        ohlcv_1m_root=choose_optional_root(task.get("ohlcv_1m_root", "")),
        duplicate_excess_warn_pct=float(task["duplicate_excess_warn_pct"]),
        duplicate_excess_hard_pct=float(task["duplicate_excess_hard_pct"]),
        daily_range_tolerance_pct=float(task["daily_range_tolerance_pct"]),
...
def build_outdir(base_outdir: str) -> Path:
    if str(base_outdir).strip():
        ...
        outdir = Path(r"C:\TSIS_Data\02_backtest_SmallCaps\runs\backtest\trades_v2_validation") / f"{stamp}_trades_v2_validation"
...
def apply_filters(df: pd.DataFrame, args: argparse.Namespace) -> pd.DataFrame:
    ...
    if str(args.date_from).strip():
        out = out[pd.to_datetime(out["date"]) >= pd.Timestamp(args.date_from)]
    if str(args.date_to).strip():
        out = out[pd.to_datetime(out["date"]) <= pd.Timestamp(args.date_to)]
    if int(args.limit) > 0:
...
def write_manifest_exports(outdir: Path, manifest: dict[str, Any]) -> None:
    ...
    manifest_df.to_csv(outdir / "batch_manifest_trades_v2.csv", index=False)
    if not manifest_df.empty:
        manifest_df.to_parquet(outdir / "batch_manifest_trades_v2.parquet", index=False)
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Validate trades inventory in parallel and write append-only batch events")
    ap.add_argument("--inventory-parquet", required=True)
    ap.add_argument("--outdir", default="")
    ap.add_argument("--run-id", default="manual_trades_v2_v020")
    ...
    ap.add_argument("--ohlcv-daily-root", default=r"D:\ohlcv_daily")
    ap.add_argument("--ohlcv-1m-root", default=r"D:\ohlcv_1m")
    ...
    manifest["ohlcv_daily_root"] = str(args.ohlcv_daily_root)
    manifest["ohlcv_1m_root"] = str(args.ohlcv_1m_root)
    manifest["filters"] = {
        "root": args.root,
        "ticker": args.ticker,
    ...
    if selected_df.empty:
        summary = {
            ...
            "ohlcv_daily_root": str(args.ohlcv_daily_root),
            "ohlcv_1m_root": str(args.ohlcv_1m_root),
    ...
    for idx, batch_df in enumerate(batches, start=1):
        ...
        for row in batch_df.to_dict(orient="records"):
            ...
            row["ohlcv_daily_root"] = args.ohlcv_daily_root
            row["ohlcv_1m_root"] = args.ohlcv_1m_root
            row["duplicate_excess_warn_pct"] = args.duplicate_excess_warn_pct
            row["duplicate_excess_hard_pct"] = args.duplicate_excess_hard_pct
            row["daily_range_tolerance_pct"] = args.daily_range_tolerance_pct
            row["min_expected_price"] = args.min_expected_price
            tasks.append(row)

        results: list[dict[str, Any]] = []
        with ProcessPoolExecutor(max_workers=int(args.workers)) as ex:
            futures = [ex.submit(worker_validate, task) for task in tasks]
