...
def parse_json_field(value: Any) -> Any:
    ...
    try:
        ...
        # Recover arrays serialized in numpy/pandas style:
        # ['a' 'b'] or ['a'\n 'b']
...
def enrich_current_with_inventory(current_df: pd.DataFrame, inventory_df: pd.DataFrame) -> pd.DataFrame:
    ...
    inv_cols = ["file", "root", "root_path", "relpath", "ticker", "year", "task_key", "size_bytes", "mtime_utc"]
...
def make_live_status(
    ...
    return {
        ...
        "current_materialized_from": str(outdir / "daily_current.parquet"),
...
def write_outputs(outdir: Path, current_df: pd.DataFrame, retry_current_df: pd.DataFrame, retry_frozen_df: pd.DataFrame, live_status: dict[str, Any]) -> None:
    ...
    current_write.to_parquet(outdir / "daily_current.parquet", index=False)
    current_write.to_csv(outdir / "daily_current.csv", index=False)
    ...
    current_write.to_csv(outdir / "daily_agent_strict_events_current.csv", index=False)
    retry_current_write.to_csv(outdir / "retry_queue_daily_strict_current.csv", index=False)
    retry_frozen_write.to_csv(outdir / "retry_frozen_daily_strict.csv", index=False)

    (outdir / "live_status_daily_strict.json").write_text(json.dumps(live_status, indent=2), encoding="utf-8")


def main() -> None:
    ap = argparse.ArgumentParser(description="Materialize current/retry/live state from daily v2 append-only events")
    ap.add_argument("--validation-outdir", required=True)
    ap.add_argument("--inventory-parquet", required=True)
    ap.add_argument("--outdir", default="")
    ap.add_argument("--run-id", default="daily_v2_materialize")
    args = ap.parse_args()
