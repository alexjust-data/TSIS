...
def derive_current(events_df: pd.DataFrame) -> pd.DataFrame:
    ...
    sort_cols = [c for c in ["file", "processed_at_utc", "batch_id"] if c in events_df.columns]
    out = events_df.sort_values(sort_cols).drop_duplicates(subset=["file"], keep="last").reset_index(drop=True)
...
def summarize_lists(current_df: pd.DataFrame, list_col: str, out_col: str) -> pd.DataFrame:
    if current_df.empty or list_col not in current_df.columns:
        return pd.DataFrame(columns=[out_col, "files"])
    rows: list[str] = []
    for value in current_df[list_col].tolist():
        parsed = value if isinstance(value, list) else parse_json_field(value)
        if isinstance(parsed, list):
            rows.extend([str(x) for x in parsed if str(x).strip()])
    if not rows:
        return pd.DataFrame(columns=[out_col, "files"])
    s = pd.Series(rows, name=out_col)
    return (
        s.value_counts(dropna=False)
        .rename_axis(out_col)
        .reset_index(name="files")
        .sort_values(["files", out_col], ascending=[False, True])
...
def make_live_status(
    ...
    return {
        ...
        "current_materialized_from": str(outdir / "ohlcv_1m_current.parquet"),
...
def write_outputs(
    ...
    current_write.to_parquet(outdir / "ohlcv_1m_current.parquet", index=False)
    current_write.to_csv(outdir / "ohlcv_1m_current.csv", index=False)
    ...
    severity_df.to_parquet(outdir / "severity_counts_current.parquet", index=False)
    severity_df.to_csv(outdir / "severity_counts_current.csv", index=False)
    issues_df.to_parquet(outdir / "issue_counts_current.parquet", index=False)
    issues_df.to_csv(outdir / "issue_counts_current.csv", index=False)
    warns_df.to_parquet(outdir / "warn_counts_current.parquet", index=False)
    warns_df.to_csv(outdir / "warn_counts_current.csv", index=False)
    by_ticker_df.to_parquet(outdir / "by_ticker_current.parquet", index=False)
    by_ticker_df.to_csv(outdir / "by_ticker_current.csv", index=False)
    by_year_df.to_parquet(outdir / "by_year_current.parquet", index=False)
    by_year_df.to_csv(outdir / "by_year_current.csv", index=False)
    by_month_df.to_parquet(outdir / "by_month_current.parquet", index=False)
    by_month_df.to_csv(outdir / "by_month_current.csv", index=False)

    current_write.to_csv(outdir / "ohlcv_1m_agent_strict_events_current.csv", index=False)
    retry_current_write.to_csv(outdir / "retry_queue_ohlcv_1m_strict_current.csv", index=False)
    retry_frozen_write.to_csv(outdir / "retry_frozen_ohlcv_1m_strict.csv", index=False)

    (outdir / "live_status_ohlcv_1m_strict.json").write_text(json.dumps(live_status, indent=2), encoding="utf-8")


def main() -> None:
    ap = argparse.ArgumentParser(description="Materialize current/retry/live state from ohlcv_1m v2 append-only events")
    ap.add_argument("--validation-outdir", required=True)
    ap.add_argument("--inventory-parquet", required=True)
    ap.add_argument("--outdir", default="")
    ap.add_argument("--run-id", default="ohlcv_1m_v2_materialize")
    args = ap.parse_args()
    ...
    severity_df = (
        current_df["severity"].value_counts(dropna=False).rename_axis("severity").reset_index(name="files")
        if not current_df.empty and "severity" in current_df.columns
        else pd.DataFrame(columns=["severity", "files"])
    )
    issues_df = summarize_lists(current_df, "issues", "issue")
    warns_df = summarize_lists(current_df, "warns", "warn")

    by_ticker_df = (
        current_df.groupby(["ticker", "severity"], dropna=False).size().reset_index(name="files").sort_values(["files", "ticker"], ascending=[False, True]).reset_index(drop=True)
        if not current_df.empty and {"ticker", "severity"}.issubset(current_df.columns)
        else pd.DataFrame(columns=["ticker", "severity", "files"])
    )
    by_year_df = (
        current_df.groupby(["year", "severity"], dropna=False).size().reset_index(name="files").sort_values(["year", "severity"]).reset_index(drop=True)
        if not current_df.empty and {"year", "severity"}.issubset(current_df.columns)
        else pd.DataFrame(columns=["year", "severity", "files"])
    )
    by_month_df = (
        current_df.groupby(["year", "month", "severity"], dropna=False).size().reset_index(name="files").sort_values(["year", "month", "severity"]).reset_index(drop=True)
        if not current_df.empty and {"year", "month", "severity"}.issubset(current_df.columns)
        else pd.DataFrame(columns=["year", "month", "severity", "files"])
    )
