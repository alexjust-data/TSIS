...
def build_once(run_id: str, run_dir: Path, expected_csv_arg: str) -> dict[str, Any]:
    ...
    download_by_ticker = pd.DataFrame(columns=['ticker', 'download_ok_tasks', 'downloaded_empty_tasks_ticker', 'download_fail_tasks_ticker', 'download_terminal_tasks'])
    if not dcur.empty and 'ticker' in dcur.columns and 'status' in dcur.columns:
        dcur['ticker'] = dcur['ticker'].astype(str).str.upper().str.strip()
        status = dcur['status'].astype(str)
        dcur = dcur.assign(
            download_ok_flag=status.eq('DOWNLOADED_OK'),
            download_empty_flag=status.eq('DOWNLOADED_EMPTY'),
            download_fail_flag=status.eq('DOWNLOAD_FAIL'),
        )
        download_by_ticker = (
            dcur.groupby('ticker', dropna=False)
            .agg(
                download_ok_tasks=('download_ok_flag', 'sum'),
                downloaded_empty_tasks_ticker=('download_empty_flag', 'sum'),
                download_fail_tasks_ticker=('download_fail_flag', 'sum'),
            )
            .reset_index()
        )
        download_by_ticker['download_terminal_tasks'] = (
            download_by_ticker['download_ok_tasks']
            + download_by_ticker['downloaded_empty_tasks_ticker']
            + download_by_ticker['download_fail_tasks_ticker']
        )
        cov = cov.merge(download_by_ticker, on='ticker', how='outer')
    ...
    cov['download_ok_tasks'] = pd.to_numeric(cov.get('download_ok_tasks', 0), errors='coerce').fillna(0).astype(int)
    cov['downloaded_empty_tasks_ticker'] = pd.to_numeric(cov.get('downloaded_empty_tasks_ticker', 0), errors='coerce').fillna(0).astype(int)
    cov['download_fail_tasks_ticker'] = pd.to_numeric(cov.get('download_fail_tasks_ticker', 0), errors='coerce').fillna(0).astype(int)
    cov['download_terminal_tasks'] = pd.to_numeric(cov.get('download_terminal_tasks', 0), errors='coerce').fillna(0).astype(int)
    cov['expected_days'] = pd.to_numeric(cov['expected_days'], errors='coerce').fillna(cov['download_terminal_tasks']).astype(int)
    ...
    cov['observed_flag'] = cov['download_terminal_tasks'] > 0
    cov['validated_flag'] = cov['files'] > 0

    def _ticker_review_status(r: pd.Series) -> str:
        if not bool(r['observed_flag']):
            return 'NOT_OBSERVED_YET'
        if not bool(r['validated_flag']):
            return 'DOWNLOADED_NOT_YET_VALIDATED'
        if r['review_queue_files'] > 0:
            return 'REVIEW_QUEUE_PENDING'
        if r['hard_fail_files'] > 0:
            return 'HARD_FAIL_PRESENT'
        return 'OBSERVED_ACCEPTED'

    cov['ticker_review_status'] = cov.apply(_ticker_review_status, axis=1)
    cov['backtest_ml_ready'] = (
        cov['validated_flag']
        & cov['hard_fail_files'].eq(0)
        & cov['review_queue_files'].eq(0)
    )
    cov = cov.sort_values(['observed_flag', 'validated_flag', 'review_queue_files', 'hard_fail_files', 'ticker'], ascending=[False, False, False, False, True]).reset_index(drop=True)

    observed_status_by_ticker = cov[cov['observed_flag']].copy().sort_values(['validated_flag', 'review_queue_files', 'hard_fail_files', 'download_terminal_tasks', 'ticker'], ascending=[False, False, False, False, True]).reset_index(drop=True)
    ...
    summary = {
        ...
        'tickers_observed': int(cov[cov['observed_flag']]['ticker'].nunique()),
        'tickers_validated': int(cov[cov['validated_flag']]['ticker'].nunique()),
...
def main() -> None:
    ...
    while True:
        ...
        print(json.dumps({
            ...
            'tickers_validated': summary['tickers_validated'],
