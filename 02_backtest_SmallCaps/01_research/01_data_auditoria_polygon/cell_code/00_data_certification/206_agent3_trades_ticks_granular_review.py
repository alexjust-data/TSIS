...
STATUS_ORDER = [
    ...
    "SOFT_FAIL",
    "HARD_FAIL",
    "DOWNLOADED_OK_PENDING_VALIDATION",
    "DOWNLOADED_EMPTY",
    "DOWNLOAD_FAIL",
    "EXPECTED_MISSING",
]
STATUS_COLOR = {
    ...
    "SOFT_FAIL": "#f4a259",
    "HARD_FAIL": "#bc4749",
    "DOWNLOADED_OK_PENDING_VALIDATION": "#3d5a80",
...
def _build_task_status(expected: pd.DataFrame, download: pd.DataFrame, valid: pd.DataFrame) -> pd.DataFrame:
    ...
    def final_status(r: pd.Series) -> str:
        ...
        if sev in ("PASS", "SOFT_FAIL", "HARD_FAIL"):
            return sev
        if dst == "DOWNLOADED_OK":
            return "DOWNLOADED_OK_PENDING_VALIDATION"
...
def _save_outputs(task_df: pd.DataFrame) -> None:
    ...
    by_ticker["found_files"] = by_ticker[["PASS", "SOFT_FAIL", "HARD_FAIL", "DOWNLOADED_OK_PENDING_VALIDATION"]].sum(axis=1)
    ...
    by_ticker["pending_validation_ratio"] = np.where(by_ticker["expected_tasks"] > 0, by_ticker["DOWNLOADED_OK_PENDING_VALIDATION"] / by_ticker["expected_tasks"], np.nan)
...
def _plot_ticker_status(task_df: pd.DataFrame) -> None:
    ...
    pivot["pending_validation"] = pivot["DOWNLOADED_OK_PENDING_VALIDATION"]
    pivot = pivot.sort_values(["problem_score", "pending_validation", "DOWNLOADED_EMPTY", "HARD_FAIL", "SOFT_FAIL"], ascending=False).head(TOP_N)
    if pivot.empty:
        return
...
def _build_explorer(task_df: pd.DataFrame) -> None:
    ...
    def _render(*_args):
        ...
        with out:
            ...
            display(detail)
            if row["final_status"] in ["PASS", "SOFT_FAIL", "HARD_FAIL", "DOWNLOADED_OK_PENDING_VALIDATION"] and row.get("file_exists", False):
