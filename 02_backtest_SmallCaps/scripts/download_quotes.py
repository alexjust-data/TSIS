...
TERMINAL_OK = {"DOWNLOADED_OK", "DOWNLOADED_EMPTY", "EMPTY_CONFIRMED"}
TERMINAL_BAD = {"DOWNLOAD_PARTIAL", "DOWNLOAD_FAIL"}
RESUME_SKIP_STATUSES = {"DOWNLOADED_OK", "DOWNLOADED_EMPTY", "EMPTY_CONFIRMED"}
...
def save_current_merge(events_current_csv: Path, old_current: pd.DataFrame, new_events: pd.DataFrame) -> pd.DataFrame:
    for c in EVENT_COLUMNS:
        if c not in old_current.columns:
            old_current[c] = pd.NA
        if c not in new_events.columns:
            new_events[c] = pd.NA

    if len(new_events) == 0:
        old_current[EVENT_COLUMNS].to_csv(events_current_csv, index=False, encoding="utf-8")
        return old_current[EVENT_COLUMNS].copy()

    # New batch events are always newer than what is already in current, so we only
    # need to deduplicate inside the new batch and then overwrite by task_key.
    batch = new_events[EVENT_COLUMNS].copy()
    batch = batch.sort_values("processed_at_utc").drop_duplicates(subset=["task_key"], keep="last")

    if len(old_current) == 0:
        out = batch.reset_index(drop=True)
        out.to_csv(events_current_csv, index=False, encoding="utf-8")
        return out

    # old_current is already persisted as the latest state per task_key; avoid
    # deep-copying millions of rows on every flush.
    cur = old_current.set_index("task_key", drop=False)
    batch_idx = batch.set_index("task_key", drop=False)

    overlap = cur.index.intersection(batch_idx.index)
    if len(overlap):
        cur.loc[overlap, EVENT_COLUMNS] = batch_idx.loc[overlap, EVENT_COLUMNS]

    new_only_idx = batch_idx.index.difference(cur.index)
    if len(new_only_idx):
        new_only = batch_idx.loc[new_only_idx, EVENT_COLUMNS]
        out = pd.concat([cur, new_only], axis=0, copy=False).reset_index(drop=True)[EVENT_COLUMNS]
    else:
        out = cur.reset_index(drop=True)[EVENT_COLUMNS]
    out.to_csv(events_current_csv, index=False, encoding="utf-8")
    return out
