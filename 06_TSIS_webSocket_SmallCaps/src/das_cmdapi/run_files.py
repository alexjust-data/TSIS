...
def screener_run_root(data_root: Path, run_id: str) -> Path:
    return data_root / "screener" / "runs" / run_id
...
def ensure_run_files(run_root: Path) -> dict[str, Path]:
    ...
    paths = {
        ...
        "candidate_registry": run_root / "candidate_registry.jsonl",
    ...
    for key in ("command_transcript", "events", "candidate_registry", "capture_log"):
        paths[key].touch()
    return paths
