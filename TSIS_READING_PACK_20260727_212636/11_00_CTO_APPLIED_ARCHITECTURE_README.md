...
> Current runtime note (2026-07-27): `event_state_on_demand_bounded_execution_preflight_correction_v0_1` is closed; next gate is `event_state_on_demand_bounded_execution_v0_1`. The next run must verify `event_state_on_demand_bounded_execution_authority_bundle_v0_1.json` before creating a request or reading data. No generalized Event State execution, official dataset promotion, production or downstream use is authorized.

Status: `readme_v0_66_event_state_on_demand_bounded_execution_preflight_correction_closed`
Date: `2026-07-27`

Current runtime update: `event_state_on_demand_bounded_execution_preflight_correction_v0_1` is recorded under `03_TABLES_feature_engineering/08_RUNTIME_CAPABILITIES` as `CLOSED_PASS_PREFLIGHT_BLOCKERS_RESOLVED_NO_EXECUTION`. It preserves the first bounded Event State on-demand execution scope for `session_opened` / `exchange_session`, XNYS, 3 sessions, 3 instrument projections and at most 9 contexts, adds bounded Market State dependency consumption authority, lifecycle binding, canonical window id and jsonl output limits, and keeps generalized Event State execution, official dataset promotion, production and downstream closed. Next gate: `event_state_on_demand_bounded_execution_v0_1`.
